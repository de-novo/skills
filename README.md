...
Ground helps independent worktrees verify their changes against a shared app
...
The names are Ground, Seat, Plan, and Facts. See the [glossary](docs/glossary.md).
...
- [Understand Ground and connect a project](skills/ground/README.md)
- [Run the disposable lab](docs/evaluation/kubernetes/README.md)
- [Read the overlay adapter contract](skills/ground/references/overlay-contract.md)
...
```
   ...
   Facts         assertions: seats propose, a judge commits; time, confidence, source
                 .agents/facts.yml · <state>/facts/<slug>.jsonl  (append-only)
...
| --- | --- | --- | --- | --- |
| [Ground](skills/ground/) | where does this project run on this machine | engines, overlay envs, names | the runtime profile | choose a port, stop shared engines |
| [Seat](skills/seat/) | who sits where, on what task, and what did they report | a seat's worktree, env, journal | Ground's report | launch an agent, merge, order the work |
| [Plan](skills/plan/) | what is the work, what may start now, how many at once | the plan (an agent writes it), seats through Seat, machine slot reservations; `serve` launches each seated item's tool | the plan, the seats, the budget, the baseline's git facts | call a model, merge, hold world facts |
| [Facts](skills/facts/) | what does the project hold true, since when, on whose word | one log line per propose, commit, invalidate | the log, a seat's report (read-only seam) | judge, write the plan, touch a seat |
...
seam is `propose --from-seat`. Reasons: [`docs/facts-design.md`](docs/facts-design.md).
...
The one launcher this catalog offers is `plan serve`, a foreground
...
\"seat\"" means, in a host that has no such tool: read that skill's
`SKILL.md` from the installed copy and follow it. A seat finds Seat's skill at
`$SEAT_SKILL`; the other skills sit beside it.
...
| Name | One line |
| --- | --- |
| [Plan](skills/plan/) | Grill the person, split the work into a plan, set how many run at once here, keep that many seated through Seat; serve holds each seat's real session |
| [clearing](skills/clearing/) | Stop and re-pitch where the work has got to, from the graph and the facts, in plain words |

## CLI

Ground's machine-engine, profile, and overlay-lifecycle tool (the ground skill). Once per **this
...
`seat`, `plan`, `understory`, `facts`, `herbarium`). `infra k3d` is the Cluster seat command (k3d/k3s). Projects choosing this machine
...
`de-novo skills canopy [--port N] [--once]` shows read-only [Canopy worktree cards, skill activity, files and overlaps](skills/seat/references/seats.md#canopy).

Engine table and ports: [`infra/README.md`](infra/README.md).
Overlay lifecycle and cleanup contract:
[`skills/ground/references/overlay-contract.md`](skills/ground/references/overlay-contract.md).
...
.agents/         skill load adapter (symlinks into skills/), and this catalog's own
                 seat-profile.yml and facts.yml (it seats its own workers)
skills/          skill sources. add a skill as <name>/SKILL.md; a short README next to it
  ...
  ground/         Ground: engines, names, overlays (directory name stays ground)
  seat/         Seat on that base (no agent launch); Canopy is one of its verbs
  plan/      Plan, budget, allocator over seats; serve holds sessions
  understory/    the work graph drawn and written up for people
  facts/      Facts: what the project holds true, apart from the plan
