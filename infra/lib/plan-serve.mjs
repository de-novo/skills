// Plan's daemon: the one thing in this catalog that owns terminals. It
...
// `assign` would not; everything it does is readable from `plan status`.
//
...
import { parseSeatCliArgs, readSeatState, runSeat } from './seat.mjs';
import { loadPlan, reclaimStaleSlots, seatArguments, seatChosen, sessionsPath } from './plan.mjs';
...
// The seat carries the Seat skill's path, as `seat seat --env` does.
const SEAT_SKILL = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../../skills/seat/SKILL.md');
const POLL_MS = 2000;
...
// is not hammered; plan restart <id> starts the count over.
...
export function launchReadiness({ item, seat, local, environment = process.env }) {
  ...
  if (executable == null) return { ok: false, kind: 'tool-missing', note: `${tool.command[0]} is not on PATH; install it or fix tools.${toolName}.command, then plan restart ${item.id}`, toolName, tool };
  if (seat.env === 'pending') return { ok: false, kind: 'env-pending', note: `overlay env ${item.id} is pending; serve retries seat plan --apply with backoff`, toolName, tool };
...
export class PlanServe {
  constructor({ project = null, environment = process.env, cwd = process.cwd(), pollMs = POLL_MS, log = console.log } = {}) {
    ...
    delete this.environment.SEAT_ID;
    delete this.environment.PLAN_SEAT;
  ...
  async start() {
    ...
    const loaded = loadPlan({ project: this.projectOption, environment: this.environment, cwd: this.cwd });
    ...
    this.socketPath = path.join(tmpdir(), `de-novo-plan-${createHash('sha256').update(this.snapshotFile).digest('hex').slice(0, 12)}.sock`);
    ...
    try {
      ...
      if (previous != null) {
        ...
        if (doc?.seats != null && typeof doc.seats === 'object') {
          ...
          if (live.length > 0) this.log(`plan: previous serve pid ${pid || '?'} ended with ${live.length} live session${live.length === 1 ? '' : 's'} (${live.map(([id]) => id).join(', ')}); they cannot be resumed natively and will be launched again as fresh contexts`);
        }
      }
    ...
    this.writeSnapshot();
    this.log(`■ ${this.project.slug} — plan serve pid ${process.pid} · socket ${this.socketPath}`);
    await this.tick();
  ...
  async tick() {
    ...
    try {
      ...
      for (const key of dropped) this.log(`plan: machine slot ${key} released`);
    } catch (error) {
      this.log(error.message);
    }
    let loaded = loadPlan({ project: this.projectOption, environment: this.environment, cwd: this.cwd });
    ...
    for (const id of seated.failures) this.log(`plan: seat ${id} not planned`);
    for (const hold of seated.refused) {
      if (this.lastHold !== hold.reason) this.log(`plan: ${hold.id}: ${hold.reason}`);
    }
    this.lastHold = seated.refused[0]?.reason ?? null;
    loaded = loadPlan({ project: this.projectOption, environment: this.environment, cwd: this.cwd });
    const { state } = readSeatState(this.project.slug, this.environment);
    ...
    for (const item of loaded.items) {
      ...
      if (session != null && seat != null && session.attempt != null && seat.attempt != null && session.attempt !== seat.attempt) {
        if (session.pty != null) {
          ...
          this.log(`plan: ${item.id}: attempt ${seat.attempt} seated; dropping the session of attempt ${session.attempt}`);
  ...
  // A pending overlay env is Seat's to repair through the same plan
  ...
  retryPending(item, loaded) {
    ...
    if (retry.attempts >= PENDING_RETRIES) {
      if (!this.sessions.has(item.id)) this.failSession(item, null, 'env-pending', `overlay env ${item.id} still pending after ${PENDING_RETRIES} plan --apply retries; fix the overlay, then plan restart ${item.id}`);
      return;
    }
    retry.attempts += 1;
    ...
    let code;
    try {
      code = runSeat({ options: parseSeatCliArgs(seatArguments(item, loaded)), environment: this.environment, cwd: this.cwd });
    } catch (error) {
      code = 1;
      this.log(error.message);
    }
    if (code === 0) {
      ...
      this.log(`plan: ${item.id}: overlay env ready after ${retry.attempts} retr${retry.attempts === 1 ? 'y' : 'ies'}`);
    } else {
      this.log(`plan: ${item.id}: overlay env still pending (retry ${retry.attempts}/${PENDING_RETRIES}, next in ${Math.round((retry.nextAt - Date.now()) / 1000)}s)`);
    }
  }
  ...
  failSession(item, toolName, kind, note) {
    ...
    this.log(`plan: ${item.id}: ${kind}: ${note}`);
    return record;
  }
  ...
  restart(id) {
    ...
    this.log(`plan: ${id}: restart requested; the next poll launches it again as a fresh context`);
    this.writeSnapshot();
  ...
  launch(item, seat, local) {
    ...
    this.sessions.set(item.id, record);
    // The seat's own events file, the one seat status, Canopy, and
    ...
    if (path.basename(tool.command[0]) === 'claude') {
      ...
      if (tool.pretrustWorktrees) {
        ...
        this.log(`plan: ${item.id}: trust ${trust.result === 'seeded' ? 'seeded' : trust.result} (${trust.file}; tools.${toolName}.pretrust_worktrees)`);
      } else {
        this.log(`plan: ${item.id}: trust not seeded; a trust dialog shows as needs-input, answer it with plan attach ${item.id}`);
      }
    }
    // The seat's task is the handoff Plan wrote at seating, so the tool
    // sees the same text a person reads with seat seat --task.
    ...
    env.PLAN_EVENTS = record.events;
    env.SEAT_EVENTS = record.events;
    env.SEAT_EVIDENCE = seatEvidencePath(this.project.slug, item.id, this.environment);
    if (settingsFile != null) env.SEAT_CLAUDE_SETTINGS = settingsFile;
    try {
      ...
      this.log(`plan: ${item.id}: ${record.note}`);
      return;
    }
    ...
    record.pty.onExit(({ exitCode }) => {
      ...
      if (!record.closing) record.note = `exited ${exitCode} on its own without a done report; plan restart ${item.id} launches it again`;
      record.pty = null;
      this.writeSnapshot();
    });
    this.log(`plan: ${item.id}: ${toolName} pid ${record.pid} in ${seat.worktree}`);
  }
  ...
  close(id, reason) {
    ...
    this.log(`plan: ${id}: closing (${reason})`);
  ...
  async stop() {
    ...
    if (lingering.length > 0) this.log(`plan: ${lingering.length} session${lingering.length === 1 ? '' : 's'} still running after the close grace: ${lingering.map((session) => `${session.id} pid ${session.pid}`).join(', ')}`);
    ...
    for (const file of [this.socketPath, this.snapshotFile]) {
      try {
        ...
        this.log(`plan: could not remove ${file}: ${error.message}`);
      }
    }
...
// plan restart <id>: ask the running serve to drop a session that is
...
export function runRestart({ options, environment = process.env, cwd = process.cwd() }) {
  ...
  if (sessions == null || !sessions.alive) fail(`no serve daemon for ${loaded.project.slug}; start one with plan serve.`);
  return new Promise((resolve) => {
    ...
    socket.on('close', () => {
      ...
      if (reply.error) { console.error(`plan: ${reply.error}`); resolve(1); return; }
      console.log(`■ ${loaded.project.slug} — plan restart ${options.id}\n  session   1/1 dropped (was ${reply.was}); the next poll launches it again`);
      resolve(0);
    });
    socket.on('error', (error) => { console.error(`plan: ${error.message}`); resolve(1); });
  });
}
...
export async function runServe({ options, environment = process.env, cwd = process.cwd() }) {
  const serve = new PlanServe({ project: options.project, environment, cwd, pollMs: Number(environment.PLAN_POLL_MS) || POLL_MS });
  await serve.start();
...
export function runAttach({ options, environment = process.env, cwd = process.cwd() }) {
  ...
  return new Promise((resolve) => {
    ...
    socket.on('data', (chunk) => {
      ...
      if (reply.error) { console.error(`plan: ${reply.error}`); socket.destroy(); resolve(1); return; }
      attached = true;
      process.stderr.write(`plan: attached to ${id} (${reply.state}); Ctrl-] detaches\n`);
    ...
    socket.on('error', (error) => { restore(); console.error(`plan: ${error.message}`); resolve(1); });
  });
}
