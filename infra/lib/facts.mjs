// de-novo Facts — the facts under the forest. One append-only log per
...
//
//   .agents/facts.yml               the project's domains, entity types, predicates, and judges (tracked)
//   <state>/facts/<slug>.jsonl      the log (machine-local, like Seat's registry)
...
import { acquireStateLock, loadSeatProject, readSeatFinished, readSeatState, resolveSeatProject } from './seat.mjs';

export const VALUES_RELPATH = '.agents/facts.yml';
...
export function parseFactsValues(yamlText, source = 'facts.yml') {
...
export function factsLogPath(slug, environment = process.env) {
  const override = environment.GROUND_STATE_DIR;
  const base = override != null ? path.resolve(override) : path.join(homedir(), '.dev-infra');
  if (override != null && override.length === 0) fail('GROUND_STATE_DIR must not be empty.');
  return path.join(base, 'facts', `${slug}.jsonl`);
}
...
// Every write re-reads the log under the same file lock Seat uses, so the
...
// One line per fact in the shape a seat brief takes: paste the block
...
export function loadFacts({ project = null, environment = process.env, cwd = process.cwd() }) {
  const location = resolveSeatProject({ project, environment, cwd });
  const seat = loadSeatProject(location);
  const valuesFile = path.join(seat.root, VALUES_RELPATH);
  if (!existsSync(valuesFile)) fail(`values not found: ${valuesFile}`);
  const values = parseFactsValues(readFileSync(valuesFile, 'utf8'), valuesFile);
  const file = factsLogPath(seat.slug, environment);
  const assertions = foldLog(readLog(file));
  return { project: seat, valuesFile, values, file, assertions };
}

export const SEAT_REPORTS = Object.freeze(['done', 'blocked']);

// The Plan seam: a seat that reported done or blocked becomes a
...
export function seatReport(slug, seatId, status = 'done', environment = process.env) {
  ...
  const finished = readSeatFinished(slug, environment).seats.filter((seat) => seat.id === seatId);
...
export function parseFactsCliArgs(args) {
...
// The seat whose worktree holds cwd, read from the Seat registry. A seat
// working in its own worktree is identified by where it stands, which is a
// registry fact, and needs neither --by nor SEAT_ID. That also holds in a
// playground sandbox, whose guard strips SEAT_* but leaves the worktree.
...
// Who writes is never guessed from nothing: --by, then the seat SEAT_ID
...
function actor(options, environment, { slug, cwd }) {
  ...
  if (environment.SEAT_ID) return assertWriter(`seat:${environment.SEAT_ID}`);
  const seat = seatAt(slug, cwd, environment);
  if (seat != null) return assertWriter(`seat:${seat}`);
  fail('pass --by <who> (a seat sets SEAT_ID, or runs from its worktree, and needs none).');
}
...
export function runFacts({ options, environment = process.env, cwd = process.cwd() }) {
  const loaded = loadFacts({ project: options.project, environment, cwd });
  ...
  switch (options.verb) {
    case 'propose': {
      ...
      if (options.from_seat != null) {
        ...
        input = { ...input, s: options.from_seat, p: REPORTED, o: report.detail, source: `seat seat ${options.from_seat} report at ${report.at}`, valid_from: report.at, ref: report.ref };
      }
...
export function factsHelp(cli = 'de-novo skills') {
  return `Facts: the assertion log, separate from Plan.

usage:
  ${cli} facts propose --s S --p P --o O --s-type T [--o-type T] --domain D --source SRC [--ref-commit SHA]
                          [--confidence 0..1] [--model M] [--valid-from ISO] [--by WHO] [--json]
  ${cli} facts propose --from-seat ID --s-type T --domain D [--report done|blocked] [--by WHO]
                                                  a seat's done (or blocked) report as a staging fact
  ${cli} facts amend <id> [--o O] [--confidence C] [--source SRC] [...any envelope field] [--by WHO]
                                                  a corrected copy, staging; committing it closes the original
  ${cli} facts commit <id> [--supersede] [--by WHO]   staging → active; refuses a conflict unless --supersede
  ${cli} facts invalidate <id> --reason TEXT [--by WHO]   active → invalid, valid_to = now
  ${cli} facts query [--s S] [--p P] [--o O] [--domain D] [--type T] [--below 0.5] [--since ISO]
                        [--status staging|active|invalid | --at ISO | --all] [--json | --ids | --brief]
  ${cli} facts trace <id> [--json]             the chain: what it amends or supersedes, and what did that to it
  ${cli} facts status [--json]                 counts, the log path, the project's vocabulary

Every verb takes --project ROOT; without it the Seat profile above the
current directory names the project. The writer is --by, else the seat
SEAT_ID names, else the seat whose worktree holds the current directory.
Domains, entity types, predicates (one|many), and the judges who may
commit and invalidate come from ${VALUES_RELPATH}.
Pattern: skills/facts/SKILL.md.`;
}
