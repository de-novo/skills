// Seat — one worktree per seat on Ground, one overlay env
// when the project has overlays, one task. Seat launches no agent: the
// launcher stays with the human. Ground is used only through its public CLI.
...
const SKILL_PATH = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../../skills/seat/SKILL.md');
...
export function parseSeatProfile(yamlText, source = 'seat-profile.yml') {
  ...
  // A project without Ground has no runtime-profile.yml to carry the slug, so
  // the Seat profile may carry it. With a Ground profile present the slug
  // lives there only; loadSeatProject rejects the duplicate.
  let project = null;
  if (doc.project != null) {
...
function findProfileUpward(startDir) {
  ...
  for (;;) {
    if (existsSync(path.join(current, SEAT_PROFILE_RELPATH))) return current;
...
export function resolveSeatProject({ project = null, environment = process.env, cwd = process.cwd() }) {
  let root;
  if (project != null) {
    root = path.resolve(cwd, project);
  } else if (environment.SEAT_PROJECT) {
    root = path.resolve(cwd, environment.SEAT_PROJECT);
  } else {
    root = findProfileUpward(cwd);
    if (root == null) {
      fail(`no ${SEAT_PROFILE_RELPATH} above ${cwd}; pass --project <baseline> or set SEAT_PROJECT.`);
    }
  }
  const profilePath = path.join(root, SEAT_PROFILE_RELPATH);
  if (!existsSync(profilePath)) fail(`seat profile not found: ${profilePath}`);
  return { root: realpathSync(root), profilePath: realpathSync(profilePath) };
}

export function loadSeatProject(location) {
  const seat = parseSeatProfile(readFileSync(location.profilePath, 'utf8'), location.profilePath);
  const runtimePath = path.join(location.root, RUNTIME_PROFILE_RELPATH);
  if (!existsSync(runtimePath)) {
    if (seat.project == null) {
      fail(`${location.profilePath}: no ${RUNTIME_PROFILE_RELPATH} to take project.slug from; declare project.slug here for a project without Ground.`);
    }
    return { root: location.root, seat, slug: seat.project.slug, overlayActive: false };
  }
  if (seat.project != null) {
    fail(`${location.profilePath}: project.slug is declared here and in ${RUNTIME_PROFILE_RELPATH}; keep it only in Ground's profile.`);
  }
...
export function seatStateDirectory(environment = process.env) {
  // Ground keeps overlays/<slug>.yml; Seat keeps seats/<slug>.yml. Under
  // GROUND_STATE_DIR both live below the override so the two files never collide.
  const override = environment.GROUND_STATE_DIR;
  if (override != null) {
    if (override.length === 0) fail('GROUND_STATE_DIR must not be empty.');
    return path.join(path.resolve(override), 'seats');
  }
  return path.join(homedir(), '.dev-infra', 'seats');
}

export function seatStatePath(slug, environment = process.env) {
  return path.join(seatStateDirectory(environment), `${slug}.yml`);
}
...
export function seatFinishedPath(slug, environment = process.env) {
  return path.join(seatStateDirectory(environment), `${slug}.finished.yml`);
}

export function readSeatFinished(slug, environment = process.env) {
  const file = seatFinishedPath(slug, environment);
...
function archiveFinishedSeat(slug, environment, id, seat) {
  const { file, seats } = readSeatFinished(slug, environment);
...
export function seatProjectsIndexPath(environment = process.env) {
  return path.join(seatStateDirectory(environment), 'projects.yml');
}
...
export function readSeatProjectsIndex(environment = process.env) {
  const file = seatProjectsIndexPath(environment);
...
function recordProjectRoot(slug, root, repository, environment) {
  ...
  try {
    const { index } = readSeatProjectsIndex(environment);
...
export function readSeatState(slug, environment = process.env) {
  const file = seatStatePath(slug, environment);
...
// Short read/merge/write critical section only. No git or Ground call runs
// while this lock is held. Exported for Facts, whose log sits under the
...
function updateState(slug, environment, operation) {
  ...
  const release = acquireStateLock(file, LOCK_WAIT_MS);
  try {
    const { state } = readSeatState(slug, environment);
...
// git already knows what a seat changed; Seat adds no tracking of its own.
...
// `seat: null` means somebody works in parallel and Seat does not know it.
...
function ground(args, project, environment) {
...
// Seat hostnames come from Ground's `urls --json`; Seat never renders a
...
function relayFailure(label, result) {
  ...
  console.error(`seat: ${label} exited ${result.status}.`);
}

// Reads Ground's machine-readable report. Seat does not open Ground's
// registry; `overlay status --json` is the seam and the contract owns it.
function probeOverlay(project, environment) {
  const result = ground(['status', '--json'], project, environment);
  let report;
  try {
    report = JSON.parse(result.stdout);
  } catch {
  ...
  return {
    ...
    // The report as Ground printed it, so a reader of status --json (Canopy)
    // has the one observation Seat made and need not ask Ground again.
    report,
    result,
  };
}

// Ground returns non-zero for any pending journal. An in-flight one is another
...
export function parseSeatCliArgs(args) {
  ...
  if (verb === 'plan') {
    ...
    if (options.resume && options.worktree != null) fail('--resume continues a Seat-created branch; it cannot adopt --worktree.');
...
export function cliJournalDetail(args) {
  ...
  // Ground never sees; the overlay contract keeps a digest of it, not the text.
  const digest =
...
export function recordSeatCliEvent(args, exit, environment = process.env, cwd = process.cwd()) {
  try {
    const id = environment.SEAT_ID;
    if (typeof id !== 'string' || !ID_PATTERN.test(id)) return false;
    const detail = cliJournalDetail(args);
    if (detail == null) return false;
    const project = loadSeatProject(resolveSeatProject({ environment, cwd }));
...
function resolveWorktreePlan(options, project, cwd, existing, finished) {
  ...
  if (project.seat.worktrees == null) {
    fail('seat-profile.yml has no worktrees section; pass --worktree <existing-path> to adopt one.');
  }
  const { root, branch } = project.seat.worktrees;
  ...
  if (!exists && gitBranchExists(seatBranch, project.root)) {
    fail(`branch ${seatBranch} already exists in ${project.root}; Seat never overwrites a branch. Move it aside yourself, or pass --resume to continue attempt ${previous.length} on its own branch.`);
  }
...
function assertRepositoryBinding(project, environment, repository) {
  ...
  const { state } = readSeatState(project.slug, environment);
  const bound = state.repository ?? entry?.repository ?? null;
  if (bound != null && bound !== repository) {
    fail(`project ${project.slug} is bound to repository ${bound.slice(0, 12)} (${entry?.root ?? 'root unknown'}); this checkout is repository ${repository.slice(0, 12)} (${project.root}). Give this project its own slug, or run seat rebind --apply from here after its live seats are finished.`);
  }
  return bound;
}
...
function runPlan({ options, project, environment, cwd }) {
  const { state } = readSeatState(project.slug, environment);
  const finished = readSeatFinished(project.slug, environment);
  ...
  if (envWanted != null && project.overlayCreateOn === 'attach') {
    ...
    const result = ground(['create', envWanted, '--apply'], project, environment);
    if (result.status === 0) {
  ...
  const seat = updateState(project.slug, environment, (current) => {
    ...
    journal(record, 'seat', existing == null ? 'plan' : 'plan.retry', journalLines.join('; '));
  ...
  if (movedFrom != null) {
    console.error(`seat: project index: ${project.slug} root moved from ${movedFrom} to ${project.root}; index keeps the latest.`);
  }
  ...
  if (envFailure != null) {
    ...
    console.error(`seat: seat ${options.id} kept with env pending; rerun the same plan --apply to retry.`);
    return 1;
  }
  return 0;
}

// The seat's environment for whatever launches the worker. SEAT_EVENTS is
// the file the tool's hooks append to, so the seat's state and what it is
// doing can be read by anyone (seat status, Canopy, Understory) whoever
// launched it; SEAT_CLAUDE_SETTINGS is the hooks file a Claude Code
// launcher passes as --settings.
export function seatEnvironment(seat, id, project, environment = process.env) {
  return {
    SEAT_ID: id,
    SEAT_ENV: seat.env == null || seat.env === 'pending' ? '' : seat.env,
    SEAT_BRANCH: seat.branch,
    SEAT_PROJECT: project.root,
    SEAT_SKILL: SKILL_PATH,
    SEAT_EVENTS: seatEventsPath(project.slug, id, environment),
    SEAT_EVIDENCE: seatEvidencePath(project.slug, id, environment),
    SEAT_CLAUDE_SETTINGS: seatSettingsPath(project.slug, id, environment),
  };
}
...
function runSeatVerb({ options, project, environment }) {
  const { state } = readSeatState(project.slug, environment);
        ...
        JSON.stringify(
          {
            ...
            env_vars: envVars,
            evidence_file: envVars.SEAT_EVIDENCE,
            skill: SKILL_PATH,
          },
          null,
          2
  ...
  if (!present) console.error(`seat: worktree missing: ${seat.worktree}`);
  return present ? 0 : 1;
}
...
// own evidence file (SEAT_EVIDENCE) when the worker wrote one there.
...
function runReport({ options, project, environment, cwd }) {
  ...
  if (result != null && result.outside_scope.length > 0 && !options.acceptOutsideScope) {
    ...
    console.error(
      [
        `seat: seat ${options.id} changed ${result.outside_scope.length} path${result.outside_scope.length === 1 ? '' : 's'} outside its scope (${current.scope.length === 0 ? 'read-only' : current.scope.join(' ')}):`,
        ...result.outside_scope.map((file) => `  ${file}`),
        `seat: narrow the change and report again, or a person accepts it with --accept-outside-scope.`,
      ].join('\n')
    );
    return 1;
  }
  const seat = updateState(project.slug, environment, (live) => {
    ...
    // The report line is the worker's own words, the seam Facts reads;
    // what git said about the result is a line of Seat's beside it.
    journal(record, 'seat', 'report', options.note == null ? options.status : `${options.status}: ${options.note}`);
    if (result != null) {
      journal(record, 'seat', 'result', `head ${result.head.slice(0, 12)}${result.clean ? '' : ' (uncommitted changes)'} · ${result.changed} path${result.changed === 1 ? '' : 's'}${result.outside_scope.length > 0 ? ` · outside scope accepted: ${result.outside_scope.join(' ')}` : ''}${result.evidence == null ? ' · no evidence' : ` · checks ${result.evidence.checks.length}, not measured ${result.evidence.not_measured.length}`}`);
    }
    return record;
  });
  console.log(
  ...
  if (result != null && !result.clean) {
    console.error(`seat: seat ${options.id} reported done with uncommitted changes; only what is committed at ${result.head.slice(0, 12)} can reach another seat.`);
  }
  if (result != null && result.evidence == null) {
    console.error(`seat: seat ${options.id} reported done with no evidence file at ${seatEvidencePath(project.slug, options.id, environment)} and no --evidence; the result stays unverified until a person checks it.`);
  }
  if (options.status === 'done' && seat.session == null) {
    console.error(`seat: seat ${options.id} has no session reference; if your tool exposes a session id or transcript path, run report --session <ref>.`);
  }
  return 0;
}
...
function runIntegrate({ options, project, environment }) {
  ...
  if (live != null) {
    updateState(project.slug, environment, (current) => {
      ...
      journal(seat, 'seat', 'integrate', detail);
    });
  } else {
    const release = acquireStateLock(finished.file, LOCK_WAIT_MS);
    try {
      const fresh = readSeatFinished(project.slug, environment);
      ...
      journal(fresh.seats[index], 'seat', 'integrate', detail);
...
function runFinishedStatus({ options, project, environment }) {
  const { seats } = readSeatFinished(project.slug, environment);
...
function runStatus({ options, project, environment }) {
  ...
  if (state.repository != null && state.repository !== repository) {
    problems.push(`registry bound to repository ${state.repository.slice(0, 12)}; this checkout is ${repository.slice(0, 12)} (seat rebind)`);
  }
...
function runFinish({ options, project, environment }) {
  ...
  let envResult = 'none';
  if (hasEnv) {
    const result = ground(['destroy', options.id, '--apply'], project, environment);
    if (result.status !== 0) {
      updateState(project.slug, environment, (current) => {
        journal(seatOf(current, options.id), 'seat', 'finish.destroy', `failed exit ${result.status}: ${lastLine(result.stderr) || lastLine(result.stdout)}`);
      });
      relayFailure('overlay destroy', result);
      console.error(`seat: seat ${options.id} kept; fix the overlay and rerun finish --apply.`);
      return 1;
    }
  ...
  if (!seat.owned) {
    ...
    updateState(project.slug, environment, (current) => {
      ...
      journal(record, 'seat', 'finish.worktree', `kept: uncommitted changes in ${seat.worktree}`);
    });
    console.error(`seat: ${seat.worktree} has uncommitted changes; commit or clean it, then rerun finish --apply.`);
  ...
  updateState(project.slug, environment, (current) => {
    const record = seatOf(current, options.id);
    journal(record, 'seat', 'finish', `env ${envResult}; worktree ${worktreeResult}; branch kept ${seat.branch}`);
...
// Overlay mode of an indexed project, read from its Ground profile. Absent or
...
function projectOverlayActive(slug, root) {
  ...
  try {
    ...
    console.error(`seat: ${slug}: ${error.message}`);
    return false;
  }
}
...
function runProjects({ options, environment }) {
  ...
  const projects = Object.entries(index.projects)
    ...
    .map(([slug, entry]) => {
      ...
      return {
        ...
        seats: Object.keys(readSeatState(slug, environment).state.seats).length,
        finished: readSeatFinished(slug, environment).seats.length,
  ...
  const lines = [`■ seat — projects ${projects.length}`];
...
export function runSeat({ options, environment = process.env, cwd = process.cwd() }) {
  ...
  switch (options.verb) {
    ...
    case 'seat':
      return runSeatVerb({ options, project, environment });
    case 'report':
...
export function seatHelp(cli = 'de-novo skills') {
  return `Seat: worktree, overlay env, and journal for one worker. Launches no agent.

usage:
  ${cli} seat plan   ID (--task TEXT | --task-file PATH) [--worktree PATH | --resume] [--owns CLAIM ...| --read-only]
                      [--revision TEXT] [--input ID=SHA ...] [--by LABEL] [--project ROOT] [--apply]
  ${cli} seat seat   ID [--json | --env | --shell | --task] [--project ROOT]
  ${cli} seat report ID --status working|blocked|done [--note TEXT] [--session REF] [--evidence PATH]
                      [--accept-outside-scope] [--project ROOT]   done reads SEAT_EVIDENCE when --evidence is not passed; --evidence - reads stdin
  ${cli} seat integrate ID --commit SHA [--by LABEL] [--note TEXT] [--project ROOT] [--apply]
  ${cli} seat status [ID] [--json] [--finished [--tail N]] [--project ROOT]
  ${cli} seat diff ID [--json] [--project ROOT]    the seat's work as a patch against its base, committed or not, plus untracked files
...
seat's result. finish destroys the env, removes only a clean, Seat-created
worktree, and keeps the seat's journal in the finished archive. The seat
carries SEAT_SKILL, the path to this skill, so a worker can read it from
any launcher. ROOT defaults to SEAT_PROJECT, then the nearest
