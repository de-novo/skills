// Plan's hook installer: one marked entry in each tool's own hook store,
...
// PLAN_EVENTS, which only a serve session carries; a session started any
...
export const HOOK_MARKER = 'de-novo plan hook';
export const TRUST_MARKER = 'de-novo plan hook trust';
...
// Every command is the same shape: with PLAN_EVENTS naming a file
// under Plan's own state directory, append one JSON line naming the
...
// process that sets PLAN_EVENTS to some other path gets nothing.
...
function hookCommand(event, { payload }) {
  // The seat's file (SEAT_EVENTS) is the address whoever launched the
  // tool; PLAN_EVENTS is serve's older name for the same file.
  const target = 'E="${SEAT_EVENTS:-$PLAN_EVENTS}"';
  const append = payload
    ? 'cat >> "$E" && printf \'\\n\' >> "$E"'
    : `printf '{"hook_event_name":"${event}"}\\n' >> "$E"`;
  const inside = 'case "$E" in "${GROUND_STATE_DIR:-$HOME/.dev-infra}"/seats/events/*|"${GROUND_STATE_DIR:-$HOME/.dev-infra}"/plans/*) true ;; *) false ;; esac';
...
function report(name) {
  const file = process.env.PLAN_EVENTS;
  if (!file || !name) return;
...
export function hookStores(environment = process.env) {
  ...
  return [
    ...
    {
      tool: 'grok',
      file: path.join(home, '.grok', 'hooks', 'de-novo-plan.json'),
    ...
    {
      tool: 'opencode',
      file: path.join(environment.OPENCODE_CONFIG_DIR || path.join(xdg, 'opencode'), 'plugins', 'de-novo-plan.js'),
...
// should not exist (a store Plan owns outright, after remove).
...
export function formatHooksReport(rows, { apply, remove }) {
  ...
  const lines = [`■ plan hooks${remove ? ' --remove' : ''}${apply ? ' --apply' : ''}`];
