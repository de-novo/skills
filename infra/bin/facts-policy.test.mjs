// Facts's commit policy is stated, not hidden: a project without judges
...
import { describeMode, makeAssertion, parseFactsValues } from '../lib/facts.mjs';
...
// A baseline repository with Seat and Facts values, so a seat can be
// planned, commit, report done, and be proposed as a fact with its head.
function fixture(t, values = VALUES) {
  const root = realpathSync(mkdtempSync(path.join(tmpdir(), 'facts-policy-')));
  ...
  writeFileSync(path.join(baseline, '.agents/seat-profile.yml'), stringify({ version: 1, project: { slug: 'policy-test' }, worktrees: { root: '../seats', branch: 'seat/{id}' } }));
...
test('the commit mode is implied by judges, may be declared, and a declaration that contradicts the judges is refused', () => {
  const permissive = parseFactsValues(stringify(VALUES));
  ...
  const restricted = parseFactsValues(stringify({ ...VALUES, judges: ['human:jane'] }));
  assert.deepEqual({ mode: restricted.mode, declared: restricted.mode_declared }, { mode: 'restricted', declared: false });
  const declared = parseFactsValues(stringify({ ...VALUES, judges: ['human:jane'], mode: 'restricted' }));
  ...
  assert.throws(() => parseFactsValues(stringify({ ...VALUES, mode: 'restricted' })), /mode: restricted needs judges/);
  assert.throws(() => parseFactsValues(stringify({ ...VALUES, judges: ['human:jane'], mode: 'permissive' })), /mode: permissive contradicts judges/);
  assert.throws(() => parseFactsValues(stringify({ ...VALUES, mode: 'open' })), /mode must be one of permissive, restricted/);
...
test('permissive: any named writer commits and status says so; restricted: a worker is refused and a judge commits', (t) => {
  ...
  assert.match(open.good(['facts', 'status']).stdout, /mode      permissive \(no judges declared\)/);
  const fact = open.json(['facts', 'propose', '--s', 'issue-1', '--p', 'caused-by', '--o', 'cache', '--s-type', 'issue', '--domain', 'auth', '--source', 'f:1', '--by', 'agent:worker']);
  assert.equal(open.good(['facts', 'commit', fact.id, '--by', 'agent:worker']).status, 0);
  assert.match(JSON.parse(open.good(['facts', 'status', '--json']).stdout).values.mode, /permissive/);

  const closed = fixture(t, { ...VALUES, judges: ['human:jane'], mode: 'restricted' });
  assert.match(closed.good(['facts', 'status']).stdout, /mode      restricted \(declared\): only human:jane/);
  const proposed = closed.json(['facts', 'propose', '--s', 'issue-1', '--p', 'caused-by', '--o', 'cache', '--s-type', 'issue', '--domain', 'auth', '--source', 'f:1', '--by', 'agent:worker']);
  const refused = closed.run(['facts', 'commit', proposed.id, '--by', 'agent:worker']);
  assert.equal(refused.status, 1);
  assert.match(refused.stderr, /agent:worker is not a judge/);
  assert.equal(closed.good(['facts', 'commit', proposed.id, '--by', 'human:jane']).status, 0);
});

test('a fact from a seat names the seat\'s own head; query says whether the baseline holds it yet', (t) => {
  const f = fixture(t);
  f.good(['seat', 'plan', 'w1', '--task', 'find the cause', '--apply']);
  ...
  f.good(['seat', 'report', 'w1', '--status', 'done', '--note', 'cause found', '--evidence', evidence]);
  const fact = f.json(['facts', 'propose', '--from-seat', 'w1', '--s-type', 'seat', '--domain', 'auth', '--by', 'human:jane']);
  assert.deepEqual(fact.ref, { commit: head, seat: 'w1', attempt: 1, base: gitIn(f.baseline, ['rev-parse', 'HEAD']), verified: true });
  assert.match(f.run(['facts', 'propose', '--from-seat', 'w1', '--s-type', 'seat', '--domain', 'auth', '--by', 'human:jane', '--ref-commit', head]).stderr, /--from-seat sets --ref-commit/);

  let rows = f.json(['facts', 'query', '--status', 'staging']);
  assert.equal(rows[0].ref_state, 'not-in-baseline');
  assert.match(f.good(['facts', 'query', '--status', 'staging']).stdout, new RegExp(`@${head.slice(0, 12)} \\(not in baseline\\)`));
  gitIn(f.baseline, ['merge', '--ff-only', 'seat/w1']);
  rows = f.json(['facts', 'query', '--status', 'staging']);
  assert.equal(rows[0].ref_state, 'in-baseline');
  assert.doesNotMatch(f.good(['facts', 'query', '--status', 'staging']).stdout, /not in baseline/);

  // A hand proposal names its commit too; a fact without one has no state.
  const byHand = f.json(['facts', 'propose', '--s', 'issue-2', '--p', 'caused-by', '--o', 'y', '--s-type', 'issue', '--domain', 'auth', '--source', 'f:2', '--by', 'human:jane', '--ref-commit', head]);
  assert.equal(byHand.ref.commit, head);
  const bare = f.json(['facts', 'propose', '--s', 'issue-3', '--p', 'caused-by', '--o', 'z', '--s-type', 'issue', '--domain', 'auth', '--source', 'f:3', '--by', 'human:jane']);
  const states = Object.fromEntries(f.json(['facts', 'query', '--status', 'staging']).map((row) => [row.id, row.ref_state]));
  assert.equal(states[byHand.id], 'in-baseline');
  assert.equal(states[bare.id], null);
  assert.match(f.run(['facts', 'propose', '--s', 'issue-4', '--p', 'caused-by', '--o', 'z', '--s-type', 'issue', '--domain', 'auth', '--source', 'f:3', '--by', 'human:jane', '--ref-commit', 'abc']).stderr, /ref\.commit must be a full commit sha/);
});
