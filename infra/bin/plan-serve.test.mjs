// Plan serve recovers instead of guessing: a launch is checked before
...
import { acquireServeLock, launchReadiness, resolveExecutable } from '../lib/plan-serve.mjs';
...
function fixture(t, { overlay = false, local = fixtureTool() } = {}) {
  ...
  const environment = { ...process.env, GROUND_STATE_DIR: path.join(root, 'state'), PLAN_POLL_MS: '150' };
  ...
  writeFileSync(path.join(baseline, '.agents/plan.local.yml'), local);
  ...
  const status = () => {
    const result = run(['plan', 'status', '--json']);
    try { return JSON.parse(result.stdout); } catch { return null; }
  };
  const session = (id) => status()?.items.find((item) => item.id === id)?.session ?? null;
  const children = new Set();
  const serve = (extra = {}) => {
    const child = spawn(process.execPath, [CLI, 'plan', 'serve', '--project', baseline], { env: { ...environment, ...extra }, stdio: ['ignore', 'pipe', 'pipe'] });
...
test('a missing tool is a failed session naming its kind; after the fix, restart launches it; a live session is not restarted', async (t) => {
  ...
  assert.match(f.run(['plan', 'status']).stdout, /w1  fixture  failed  · tool-missing: \/no\/such\/tool is not on PATH/);
  ...
  writeFileSync(path.join(f.baseline, '.agents/plan.local.yml'), fixtureTool());
  const restarted = f.run(['plan', 'restart', 'w1']);
...
test('a session belongs to one seat: finishing the seat closes it, and a new attempt gets its own launch', async (t) => {
  ...
  writeFileSync(path.join(f.baseline, '.agents/plan.yml'), PLAN.replace('    owns: [docs/reference/**]\n', '    owns: [docs/reference/**]\n    retry: { max_attempts: 2 }\n'));
  f.run(['seat', 'report', 'w1', '--status', 'blocked', '--note', 'wrong scope']);
  assert.equal(f.run(['seat', 'finish', 'w1', '--apply']).status, 0);
...
test('two serves started at once leave exactly one; the other stops at the lock and removes nothing', async (t) => {
  ...
  assert.equal(existsSync(path.join(f.root, 'state/plans/serve-test.yml.serve.lock')), false, 'the lock goes with the daemon');
});
...
test('a pending overlay env is never launched into: serve plans it again with backoff, gives up after five tries, and restart begins again', async (t) => {
  ...
  const serve = f.serve({ GROUND_PROCESS_TEST_ROOT: late });
  await f.until(() => /overlay env still pending \(retry 2\/5/.test(serve.log), 'second retry logged');
  const seat = JSON.parse(f.run(['seat', 'status', '--json'], { GROUND_PROCESS_TEST_ROOT: late }).stdout).seats.find((row) => row.id === 'w1');
  ...
  assert.equal(f.run(['plan', 'restart', 'w1']).status, 0);
  ...
  const after = JSON.parse(f.run(['seat', 'status', '--json'], { GROUND_PROCESS_TEST_ROOT: late }).stdout).seats.find((row) => row.id === 'w1');
