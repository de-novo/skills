...
import { claimsIntersect, claimSegments, itemRevision, itemStates, parsePlan } from '../lib/plan.mjs';
...
function fixture(t, { plan = PLAN, local = 'version: 1\nparallel: 2\n' } = {}) {
  ...
  const item = (id) => json(['plan', 'plan']).items.find((row) => row.id === id);
...
test('F01: a done report alone does not release a result dependency; the result must be in the baseline the next seat starts from', (t) => {
  ...
  const status = f.json(['seat', 'status']);
  ...
  assert.deepEqual(f.json(['plan', 'next']).next, ['web-panel']);
  const before = f.good(['plan', 'assign', '--apply']).stdout;
  ...
  gitIn(f.baseline, ['merge', '--ff-only', 'seat/define-shape']);
  ...
  const seatRecord = f.json(['seat', 'status']).seats.find((seat) => seat.id === 'api-endpoint');
  assert.equal(seatRecord.base, gitIn(f.baseline, ['rev-parse', 'HEAD']));
  assert.deepEqual(seatRecord.inputs, { 'define-shape': headA });
  const handoff = f.good(['seat', 'seat', 'api-endpoint', '--task']).stdout;
...
test('F01: a squash integration needs an explicit record naming a real commit; a false sha is refused', (t) => {
  ...
  gitIn(seatA, ['add', '.']);
  gitIn(seatA, ['commit', '-m', 'shape']);
  f.good(['seat', 'report', 'define-shape', '--status', 'done']);
  gitIn(f.baseline, ['merge', '--squash', 'seat/define-shape']);
  ...
  const refused = f.bad(['seat', 'integrate', 'define-shape', '--commit', 'ffffffffffffffffffffffffffffffffffffffff', '--apply']);
  assert.match(refused.stderr, /not a commit of this repository/);
  const plan = f.good(['seat', 'integrate', 'define-shape', '--commit', squash]).stdout;
  assert.match(plan, /would record/);
  assert.equal(f.item('api-endpoint').state, 'waiting');
  f.good(['seat', 'integrate', 'define-shape', '--commit', squash, '--by', 'human:reviewer', '--apply']);
  ...
  const archived = f.json(['seat', 'status', '--finished']).finished.find((seat) => seat.id === 'define-shape');
...
test('F01: done with uncommitted changes, or a worktree that moved after done, is not a usable result', (t) => {
  ...
  const report = f.good(['seat', 'report', 'define-shape', '--status', 'done']);
...
test('a seat has one evidence file, named by its environment and the handoff; a done report reads it without --evidence', (t) => {
  const f = fixture(t);
  f.good(['plan', 'assign', '--apply']);
  const seat = f.json(['seat', 'seat', 'define-shape']);
  assert.equal(seat.evidence_file, seat.env_vars.SEAT_EVIDENCE);
  assert.match(seat.evidence_file, /\/define-shape\.evidence\.yml$/);
  assert.equal(existsSync(seat.evidence_file), false, 'the worker writes it; Seat does not');
  ...
  assert.equal(f.json(['seat', 'status']).seats[0].result.evidence.checks[0].command, 'npm test');
  ...
  f.good(['plan', 'assign', '--apply']);
  const piped = spawnSync(process.execPath, [CLI, 'seat', 'report', 'web-panel', '--status', 'done', '--evidence', '-', '--project', f.baseline], { cwd: f.baseline, env: f.environment, encoding: 'utf8', input: stringify({ checks: [{ command: 'node --check x.mjs', exit: 0, observed: 'no output' }], not_measured: [{ boundary: 'browser', reason: 'none here' }] }) });
  assert.equal(piped.status, 0, piped.stderr);
  const record = f.json(['seat', 'status']).seats.find((row) => row.id === 'web-panel');
  ...
  const empty = spawnSync(process.execPath, [CLI, 'seat', 'report', 'web-panel', '--status', 'done', '--evidence', '-', '--project', f.baseline], { cwd: f.baseline, env: f.environment, encoding: 'utf8', input: '' });
...
test('F03/WP-03: evidence is recorded as given and shown as unverified when absent', (t) => {
  ...
  assert.match(f.bad(['seat', 'report', 'define-shape', '--status', 'done', '--evidence', path.join(f.root, 'bad.yml')]).stderr, /checks\[0\]\.exit/);
  f.good(['seat', 'report', 'define-shape', '--status', 'done', '--evidence', evidence]);
  const seat = f.json(['seat', 'status']).seats.find((row) => row.id === 'define-shape');
...
test('F04: a new attempt takes a fresh branch and keeps the failed one; --resume continues the previous branch', (t) => {
  ...
  const second = f.good(['plan', 'assign', '--apply']);
  assert.match(second.stdout, /assigned 1\/1/);
  const record = f.json(['seat', 'status']).seats.find((seat) => seat.id === 'define-shape');
  assert.equal(record.branch, 'seat/define-shape-2');
  ...
  assert.equal(gitIn(f.baseline, ['rev-parse', '--verify', 'refs/heads/seat/define-shape']).length, 40, 'the failed branch is kept');
  f.good(['seat', 'finish', 'define-shape', '--apply']);

  // A resumed attempt sits on the previous branch, base unchanged.
  const resumed = f.good(['seat', 'plan', 'define-shape', '--task', 'continue', '--resume', '--apply']).stdout;
  assert.match(resumed, /resumes attempt 2/);
  const again = f.json(['seat', 'status']).seats.find((seat) => seat.id === 'define-shape');
  assert.equal(again.branch, 'seat/define-shape-2');
...
test('F07: claims are normalized and validated; alias spellings are one claim', () => {
  ...
  assert.throws(() => parsePlan(plan('/etc/passwd')), /owns\[0\] must be relative/);
  assert.throws(() => parsePlan(plan('../other/**')), /owns\[0\] must not leave the repository/);
  assert.throws(() => parsePlan(plan('src/../src/api')), /owns\[0\] must not leave the repository/);
  assert.throws(() => parsePlan(plan('src/{a,b}/**')), /owns\[0\] uses an unsupported glob/);
  assert.deepEqual(parsePlan(plan('./src//api/')).tasks[0].owns, ['src/api']);
...
test('F07: a done report is refused while the seat changed paths outside its scope; renames count on both sides', (t) => {
  ...
  f.good(['plan', 'assign', '--apply']);
  const seatB = f.seat('api-endpoint');
  const scope = f.json(['seat', 'status']).seats.find((seat) => seat.id === 'api-endpoint').scope;
  ...
  const refused = f.bad(['seat', 'report', 'api-endpoint', '--status', 'done']);
  ...
  assert.equal(f.json(['seat', 'status']).seats.find((seat) => seat.id === 'api-endpoint').status, 'planned', 'a refused report changes no status');
  const accepted = f.good(['seat', 'report', 'api-endpoint', '--status', 'done', '--accept-outside-scope']);
  assert.match(accepted.stdout, /done/);
  const record = f.json(['seat', 'status']).seats.find((seat) => seat.id === 'api-endpoint');
...
async function serveOnce(t, f, { pretrust = false }) {
  ...
  writeFileSync(path.join(f.baseline, '.agents/plan.local.yml'), `version: 1\nparallel: 1\ntool: claude\ntools:\n  claude:\n    command: [${JSON.stringify(wrapper)}, "{task}"]\n${pretrust ? '    pretrust_worktrees: true\n' : ''}`);
  const environment = { ...f.environment, CLAUDE_CONFIG_DIR: configDir, PLAN_POLL_MS: '300' };
  const serve = spawn(process.execPath, [CLI, 'plan', 'serve', '--project', f.baseline], { env: environment, stdio: ['ignore', 'pipe', 'pipe'] });
  let log = '';
  ...
  while (Date.now() < deadline) {
    const result = spawnSync(process.execPath, [CLI, 'plan', 'status', '--json', '--project', f.baseline], { env: environment, encoding: 'utf8' });
