// The Facts arc at its real boundary, every time the suite runs: a
// playground sandbox, a real Seat seat in its worktree, the writes a
...
import { seatStateDirectory } from '../lib/seat.mjs';
import { factsLogPath } from '../lib/facts.mjs';
...
// listener on a kernel port, a Ground profile, a Seat profile. Nothing
// here needs Docker.
function fixture(t) {
  ...
  const env = { ...environment, GROUND_STATE_DIR: path.join(sandbox, 'state') };
  t.after(async () => {
...
test('the arc: a seat proposes from its worktree, a judge commits, the reads answer, Understory points, down leaves no trace', async (t) => {
  ...
  writeFileSync(path.join(project, '.agents/facts.yml'), VALUES);
  writeFileSync(path.join(project, '.agents/plan.yml'), PLAN);
  ...
  const seat = (args) => ok(['facts', ...args], { cwd: worktree });
  const seatId = (args) => JSON.parse(seat([...args, '--json'])).id;
  const judge = (args) => ok(['facts', ...args, '--project', project, '--by', 'human:reader']);
  const machineBefore = { seat: seatStateDirectory({}), log: factsLogPath('playground', {}) };
  const mentions = () => [machineBefore.seat, path.dirname(machineBefore.log)]
    .filter(existsSync)
  ...
  ok(['seat', 'plan', 'worker', '--project', project, '--task', 'Change the sample web page', '--by', 'reader', '--apply']);
  assert.equal(existsSync(worktree), true);
  const e1 = seatId(['propose', '--s', 'reviewer', '--p', 'depends-on', '--o', 'worker', '--s-type', 'item', '--o-type', 'item', '--domain', 'sample', '--source', '.agents/plan.yml']);
  assert.equal(JSON.parse(seat(['query', '--status', 'staging', '--json']))[0].agent_id, 'seat:worker');
  refused(['facts', 'commit', e1], /seat:worker is not a judge of this project \(judges: human:reader\)/, { cwd: worktree });
  refused(['facts', 'propose', '--s', 'x', '--p', 'y', '--o', 'z', '--s-type', 'item', '--domain', 'sample', '--source', 'f', '--project', project], /pass --by <who>/);

  // A13: an undeclared predicate; A14: an undeclared type.
  refused(['facts', 'propose', '--s', 'worker', '--p', 'caused_by', '--o', 'x', '--s-type', 'item', '--domain', 'sample', '--source', 'f'], /predicate "caused_by" is not in .*\(depends-on, uses, passed, reported\)/, { cwd: worktree });
  refused(['facts', 'propose', '--s', 'worker', '--p', 'uses', '--o', 'x', '--s-type', 'module', '--domain', 'sample', '--source', 'f'], /s_type "module" is not in/, { cwd: worktree });
  ...
  refused(['facts', 'commit', dup, '--project', project, '--by', 'human:reader'], new RegExp(`${e1} already states reviewer depends-on worker`));
  ...
  judge(['commit', u1]);
  refused(['facts', 'commit', u2, '--project', project, '--by', 'human:reader'], new RegExp(`conflicts with active ${u1}`));
  ...
  ok(['seat', 'report', 'worker', '--project', project, '--status', 'blocked', '--note', 'waits for review']);
  ...
  refused(['facts', 'propose', '--from-seat', 'worker', '--s-type', 'seat', '--domain', 'sample', '--project', project, '--by', 'human:reader'], /has no done report/);
  ok(['seat', 'report', 'worker', '--project', project, '--status', 'done', '--note', 'page changed']);
  ...
  const active = JSON.parse(ok(['facts', 'query', '--project', project, '--json']));
  assert.deepEqual(active.map((row) => row.id).sort(), [e1, e2, fixed].sort());
  assert.deepEqual(JSON.parse(ok(['facts', 'query', '--project', project, '--at', '2026-09-03T00:00:00Z', '--json'])).map((row) => row.id), [u1]);
  assert.deepEqual(JSON.parse(ok(['facts', 'query', '--project', project, '--at', '2026-09-06T00:00:00Z', '--json'])).map((row) => row.id), [fixed]);
  assert.equal(JSON.parse(ok(['facts', 'query', '--project', project, '--s', 'sample-web', '--all', '--json'])).length, 3);
  const chain = JSON.parse(ok(['facts', 'trace', fixed, '--project', project, '--json']));
  assert.deepEqual(chain.map((row) => [row.id, row.link]), [[u1, `superseded by ${u2}`], [u2, `supersedes ${u1}`], [fixed, `amends ${u2}`]]);
  assert.equal(JSON.parse(ok(['facts', 'query', '--project', project, '--since', '2100-01-01T00:00:00Z', '--json'])).length, 0);
  // Staging: the duplicate edge, the check, the blocked report, the done report.
  assert.equal(ok(['facts', 'query', '--project', project, '--status', 'staging', '--ids']).trim().split('\n').length, 4);
  const brief = ok(['facts', 'query', '--project', project, '--brief', '--s', 'reviewer']);
  assert.match(brief, new RegExp(`^- ${e1}  reviewer depends-on worker  \\(sample, 0.50, .agents/plan.yml\\)$`, 'm'));
  assert.match(brief, new RegExp(`^- ${e2}  reviewer depends-on design-note`, 'm'));
  const status = JSON.parse(ok(['facts', 'status', '--project', project, '--json']));
  assert.deepEqual(status.counts, { total: 9, staging: 4, active: 3, invalid: 2, staging_below_half: 0 });
  assert.equal(status.file, path.join(f.sandbox, 'state/facts/playground.jsonl'));
  ...
  assert.equal(out.machine.seat.length + out.machine.overlays.length, 0);
