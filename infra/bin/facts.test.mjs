...
const PARSED = parseFactsValues(stringify(VALUES));

// A disposable baseline with a Seat profile that carries its own slug (no
// Ground profile, as this catalog itself does), a vocabulary, and its own
// state root. Nothing is seated; the log is the thing under test.
function fixture(t, { values = VALUES } = {}) {
  const root = realpathSync(mkdtempSync(path.join(tmpdir(), 'facts-')));
  ...
  writeFileSync(path.join(baseline, '.agents/seat-profile.yml'), stringify({ version: 1, project: { slug: 'facts-test' }, worktrees: { root: '../seats', branch: 'seat/{id}' } }));
  writeFileSync(path.join(baseline, '.agents/facts.yml'), stringify(values));
  t.after(() => rmSync(root, { recursive: true, force: true }));
  const run = (args, env = {}) => spawnSync(process.execPath, [CLI, 'facts', ...args], { cwd: baseline, encoding: 'utf8', env: { ...environment, ...env } });
  const json = (args, env = {}) => {
    const result = run([...args, '--json'], env);
    assert.equal(result.status, 0, result.stderr || result.stdout);
    return JSON.parse(result.stdout);
  };
  return { root, baseline, environment, run, json, log: path.join(root, 'state/facts/facts-test.jsonl') };
}
...
test('the values file is a whitelist: unknown keys, empty lists, and bad tokens are refused', () => {
  const ok = parseFactsValues(stringify(VALUES));
  assert.deepEqual(ok.domains, ['sprint', 'auth']);
  assert.throws(() => parseFactsValues(stringify({ ...VALUES, judge: 'fable' })), /unknown key "judge"/);
  assert.throws(() => parseFactsValues(stringify({ ...VALUES, version: 2 })), /version must be 1/);
  assert.throws(() => parseFactsValues(stringify({ ...VALUES, domains: [] })), /domains must be a non-empty list/);
  assert.throws(() => parseFactsValues(stringify({ ...VALUES, types: ['Issue'] })), /types entry "Issue" must be a lower-case token/);
  assert.throws(() => parseFactsValues(stringify({ ...VALUES, types: ['issue', 'issue'] })), /types has a duplicate/);
  // judges are optional; when named they are writer ids, and only they may commit.
  assert.equal(ok.judges, null);
  const judged = parseFactsValues(stringify({ ...VALUES, judges: ['human:jane', 'agent:judge'] }));
  assert.deepEqual(judged.judges, ['human:jane', 'agent:judge']);
  assert.throws(() => parseFactsValues(stringify({ ...VALUES, judges: [] })), /judges must be a non-empty list/);
  assert.throws(() => parseFactsValues(stringify({ ...VALUES, judges: ['Jane Doe'] })), /judges entry "Jane Doe" must be a writer id/);
  ...
  assert.throws(() => parseFactsValues(stringify(noPredicates)), /predicates must be a non-empty map of name: one\|many/);
  assert.throws(() => parseFactsValues(stringify({ ...VALUES, predicates: {} })), /predicates must be a non-empty map/);
  assert.throws(() => parseFactsValues(stringify({ ...VALUES, predicates: ['caused-by'] })), /predicates must be a non-empty map/);
  assert.throws(() => parseFactsValues(stringify({ ...VALUES, predicates: { 'Caused-By': 'one' } })), /predicate "Caused-By" must be a lower-case token/);
  assert.throws(() => parseFactsValues(stringify({ ...VALUES, predicates: { 'caused-by': 'single' } })), /predicate caused-by must be one or many, got "single"/);
  assert.throws(() => parseFactsValues(stringify({ ...VALUES, predicates: { reported: 'one' } })), /predicate "reported" is built in \(many\) and may not be redeclared/);
});
...
test('an assertion needs subject, predicate, object, a declared type and domain, a source and an agent', () => {
  ...
  assert.throws(() => fact({ p: 'caused_by' }), /predicate "caused_by" is not in \.agents\/facts\.yml predicates \(caused-by, done-when, depends-on, b, reported\)/);
  assert.throws(() => fact({ s_type: 'module' }), /s_type "module" is not in \.agents\/facts\.yml types/);
  assert.throws(() => fact({ o_type: 'module' }), /o_type "module" is not in/);
  assert.throws(() => fact({ domain: 'payments' }), /domain "payments" is not in \.agents\/facts\.yml domains/);
...
test('the CLI parser knows each verb, its flags, and what propose must be given', () => {
  assert.equal(parseFactsCliArgs([]).help, true);
  assert.equal(parseFactsCliArgs(['help']).help, true);
  assert.throws(() => parseFactsCliArgs(['prune']), /unknown command "prune"/);
  const full = parseFactsCliArgs(['propose', '--s', 'x', '--p', 'is', '--o', 'y', '--s-type', 'issue', '--domain', 'auth', '--source', 'f:1', '--confidence', '0.8']);
  assert.equal(full.s_type, 'issue');
  assert.equal(full.confidence, '0.8');
  assert.throws(() => parseFactsCliArgs(['propose', '--s', 'x', '--p', 'is', '--o', 'y', '--s-type', 'issue', '--domain', 'auth']), /propose requires --source/);
  assert.throws(() => parseFactsCliArgs(['propose', '--s', 'x', '--p', 'is', '--o', 'y', '--source', 'f', '--domain', 'auth']), /propose requires --s-type/);
  assert.throws(() => parseFactsCliArgs(['propose', '--s', 'x', '--p', 'is', '--o', 'y', '--source', 'f', '--s-type', 'issue']), /propose requires --domain/);
  assert.throws(() => parseFactsCliArgs(['propose', '--from-seat', 'w1', '--s', 'x', '--s-type', 'seat', '--domain', 'auth']), /--from-seat sets --s from the seat's report/);
  assert.equal(parseFactsCliArgs(['propose', '--from-seat', 'w1', '--s-type', 'seat', '--domain', 'auth']).from_seat, 'w1');
  assert.throws(() => parseFactsCliArgs(['propose', '--s']), /--s requires a value/);
  assert.throws(() => parseFactsCliArgs(['propose', 'extra']), /propose takes no positional arguments/);
  assert.throws(() => parseFactsCliArgs(['commit']), /commit takes exactly 1 positional argument/);
  assert.equal(parseFactsCliArgs(['amend', 'a-1', '--o', 'y', '--confidence', '0.9']).o, 'y');
  assert.throws(() => parseFactsCliArgs(['amend']), /amend takes exactly 1 positional argument/);
  assert.throws(() => parseFactsCliArgs(['amend', 'a-1', '--from-seat', 'w1']), /--from-seat is not valid for amend/);
  assert.equal(parseFactsCliArgs(['commit', 'a-1', '--supersede']).supersede, true);
  assert.throws(() => parseFactsCliArgs(['commit', 'a-1', '--reason', 'x']), /--reason is not valid for commit/);
  assert.throws(() => parseFactsCliArgs(['invalidate', 'a-1']), /invalidate requires --reason/);
  assert.throws(() => parseFactsCliArgs(['query', '--status', 'done']), /--status must be one of staging, active, invalid/);
  assert.throws(() => parseFactsCliArgs(['query', '--status', 'active', '--at', '2026-09-08T00:00:00Z']), /pass --at or --status, not both/);
  assert.equal(parseFactsCliArgs(['query', '--ids']).ids, true);
  assert.throws(() => parseFactsCliArgs(['query', '--ids', '--json']), /pass one of --json, --ids, --brief/);
  assert.equal(parseFactsCliArgs(['status', '--project', '/x']).project, '/x');
});
...
test('the CLI round trip: status, propose, commit, query, invalidate at a real slug, with the log read back', (t) => {
  ...
  assert.equal(status.project, 'facts-test');
  ...
  // A seat names itself through SEAT_ID.
  const proposed = f.json(['propose', '--s', 'issue-12', '--p', 'caused-by', '--o', 'stale cache', '--s-type', 'issue', '--domain', 'auth', '--source', 'src/auth/cache.ts:40', '--model', 'example-model'], { SEAT_ID: 'w1' });
  ...
  assert.match(help.stdout, /facts propose --s S --p P --o O/);
  assert.match(help.stdout, /facts amend <id>/);
  ...
  assert.match(missing.stderr, /seat profile not found/);
});
...
test('with judges declared, only a judge commits or invalidates; anyone named still proposes and amends', (t) => {
  ...
  const proposed = f.json(['propose', '--s', 'issue-12', '--p', 'caused-by', '--o', 'stale cache', '--s-type', 'issue', '--domain', 'auth', '--source', 'src/auth/cache.ts:40'], { SEAT_ID: 'w1' });
  const seatCommit = f.run(['commit', proposed.id], { SEAT_ID: 'w1' });
  ...
  const amended = f.json(['amend', proposed.id, '--confidence', '0.8', '--source', 'src/auth/cache.ts:40-52'], { SEAT_ID: 'w1' });
  ...
  const seatInvalidate = f.run(['invalidate', amended.id, '--reason', 'x'], { SEAT_ID: 'w1' });
...
test('a seat working in its own worktree is the writer, with no --by and no SEAT_ID', (t) => {
  const f = fixture(t);
  ...
  mkdirSync(path.join(f.root, 'state/seats'), { recursive: true });
  const seat = { worktree, branch: 'seat/api-endpoint', base: 'abc', task: 't', created_at: '2026-09-08T00:00:00.000Z', status: 'working', owned: true, env: null, by: null, session: null, journal: [] };
  writeFileSync(path.join(f.root, 'state/seats/facts-test.yml'), stringify({ version: 1, project: 'facts-test', seats: { 'api-endpoint': seat, gone: { ...seat, worktree: path.join(f.root, 'seats/gone') } } }));

  assert.equal(seatAt('facts-test', path.join(worktree, 'src'), f.environment), 'api-endpoint');
  assert.equal(seatAt('facts-test', worktree, f.environment), 'api-endpoint');
  assert.equal(seatAt('facts-test', f.baseline, f.environment), null);
  assert.equal(seatAt('facts-test', path.join(f.root, 'seats/api-endpoint-2'), f.environment), null);
  assert.equal(seatAt('facts-test', path.join(f.root, 'nowhere'), f.environment), null);

  const inSeat = (args, env = {}) => spawnSync(process.execPath, [CLI, 'facts', ...args, '--project', f.baseline, '--json'], { cwd: path.join(worktree, 'src'), encoding: 'utf8', env: { ...f.environment, ...env } });
  ...
  // SEAT_ID still wins over the worktree, and --by over both.
  assert.equal(JSON.parse(inSeat(['propose', '--s', 'a', '--p', 'b', '--o', 'c', '--s-type', 'issue', '--domain', 'auth', '--source', 'f'], { SEAT_ID: 'w9' }).stdout).agent_id, 'seat:w9');
  assert.equal(JSON.parse(inSeat(['propose', '--s', 'a', '--p', 'b', '--o', 'd', '--s-type', 'issue', '--domain', 'auth', '--source', 'f', '--by', 'human:jane'], { SEAT_ID: 'w9' }).stdout).agent_id, 'human:jane');
...
test('a many predicate takes a second object as another edge; a one predicate takes it as a conflict; duplicates are refused for both', (t) => {
  ...
  assert.throws(() => commit({ file: f.log, values: parseFactsValues(stringify(narrowed)), id: late.id, by: 'human' }), /predicate "depends-on" is no longer in/);
  graph = foldLog(readLog(f.log));
...
test('commit takes the log lock: two committers racing on one predicate leave exactly one active, and n proposers leave n whole lines', async (t) => {
  ...
  const race = (id) => new Promise((resolve) => {
    const child = spawn(process.execPath, [CLI, 'facts', 'commit', id, '--by', 'human'], { cwd: f.baseline, env: f.environment });
    let stderr = '';
  ...
  const proposers = Array.from({ length: 12 }, (_, index) => new Promise((resolve) => {
    const child = spawn(process.execPath, [CLI, 'facts', 'propose', '--s', `issue-${index}`, '--p', 'depends-on', '--o', 'x'.repeat(200), '--s-type', 'issue', '--domain', 'auth', '--source', 'f', '--by', `seat:p${index}`], { cwd: f.baseline, env: f.environment });
    let stderr = '';
    child.stderr.on('data', (chunk) => { stderr += chunk; });
    child.on('close', (code) => resolve({ code, stderr }));
  }));
...
test('query --all, --since, --brief and trace read the chain a fact belongs to', (t) => {
  ...
  assert.throws(() => parseFactsCliArgs(['query', '--all', '--status', 'active']), /--all takes every status/);
  assert.throws(() => parseFactsCliArgs(['query', '--brief', '--ids']), /pass one of --json, --ids, --brief/);
  assert.throws(() => parseFactsCliArgs(['trace']), /trace takes exactly 1 positional argument/);
});
...
test('propose --from-seat turns a seat\'s done report into a staging fact, and refuses a seat that has not reported done', (t) => {
  const f = fixture(t);
  const seat = (status, journal) => ({ worktree: '/w', branch: 'seat/x', base: 'abc', task: 't', created_at: '2026-09-08T00:00:00.000Z', status, owned: true, env: null, by: 'codex', session: null, journal });
  mkdirSync(path.join(f.root, 'state/seats'), { recursive: true });
  writeFileSync(
    path.join(f.root, 'state/seats/facts-test.yml'),
    stringify({
      version: 1,
      project: 'facts-test',
      seats: {
        'api-endpoint': seat('done', [
          { at: '2026-09-08T01:00:00.000Z', actor: 'seat', event: 'plan', detail: 'worktree add' },
  ...
  writeFileSync(
    path.join(f.root, 'state/seats/facts-test.finished.yml'),
    stringify({ version: 1, project: 'facts-test', seats: [{ id: 'docs-pass', finished_at: '2026-09-08T04:00:00.000Z', ...seat('done', [{ at: '2026-09-08T03:30:00.000Z', actor: 'seat', event: 'report', detail: 'done' }]) }] })
  );

  assert.deepEqual(seatDoneReport('facts-test', 'docs-pass', f.environment), { at: '2026-09-08T03:30:00.000Z', detail: 'done', by: 'codex', ref: null });
  // A retried item: an archived earlier seat reported blocked, the live seat reported blocked later. The live seat wins.
  const archive = parse(readFileSync(path.join(f.root, 'state/seats/facts-test.finished.yml'), 'utf8'));
  ...
  writeFileSync(path.join(f.root, 'state/seats/facts-test.finished.yml'), stringify(archive));
  assert.equal(seatReport('facts-test', 'web-panel', 'blocked', f.environment).detail, 'blocked: waits for the shape');
  ...
  writeFileSync(path.join(f.root, 'state/seats/facts-test.finished.yml'), stringify(archive));
  assert.equal(seatDoneReport('facts-test', 'docs-pass', f.environment).detail, 'done: second run');
  assert.deepEqual(seatReport('facts-test', 'web-panel', 'blocked', f.environment), { at: '2026-09-08T01:30:00.000Z', detail: 'blocked: waits for the shape', by: 'codex', ref: null });
  assert.throws(() => seatReport('facts-test', 'api-endpoint', 'blocked', f.environment), /seat api-endpoint: has no blocked report/);
  assert.throws(() => seatReport('facts-test', 'web-panel', 'working', f.environment), /--report must be one of done, blocked/);
  // 'done-ish' does not match done: the report is the status, or the status and a colon.
  assert.throws(() => seatReport('facts-test', 'web-panel', 'done', f.environment), /has no done report/);
  assert.throws(() => seatDoneReport('facts-test', 'web-panel', f.environment), /seat web-panel: has no done report/);
  assert.throws(() => seatDoneReport('facts-test', 'nobody', f.environment), /seat nobody: not in the registry or the finished archive/);
  ...
  assert.equal(row.source, 'seat seat api-endpoint report at 2026-09-08T03:00:00.000Z');
  ...
  assert.throws(() => parseFactsCliArgs(['propose', '--from-seat', 'w', '--report', 'working', '--s-type', 'seat', '--domain', 'sprint']), /--report must be one of done, blocked/);
  assert.throws(() => parseFactsCliArgs(['propose', '--s', 'a', '--p', 'b', '--o', 'c', '--source', 'f', '--report', 'done', '--s-type', 'seat', '--domain', 'sprint']), /--report goes with --from-seat/);
  // The registry itself was only read.
  assert.match(readFileSync(path.join(f.root, 'state/seats/facts-test.yml'), 'utf8'), /status: blocked/);
});
