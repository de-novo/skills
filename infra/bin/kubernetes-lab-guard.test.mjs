...
for (const kind of ['existing cluster', 'different context', 'external kubeconfig', 'valid private target']) {
  test(`Kubernetes acceptance lab target guard: ${kind}`, () => {
    const root = mkdtempSync(path.join(tmpdir(), 'ground-lab-guard-'));
    try {
      const config = { cluster: 'ground-test-owned', context: 'k3d-ground-test-owned', kubeconfig: path.join(root, 'kubeconfig') };
      ...
      const result = spawnSync(process.execPath, [backend, 'create', 'w1', '--apply'], { env: { PATH: '', GROUND_KUBERNETES_LAB: root }, encoding: 'utf8', timeout: 3000 });
