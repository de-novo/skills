...
import { allocate, machineView, reclaimStaleSlots, reserveSlots, slotsPath } from '../lib/plan.mjs';
...
function machine(t) {
  ...
  const project = (slug) => {
    ...
    writeFileSync(path.join(baseline, '.agents/seat-profile.yml'), stringify({ version: 1, worktrees: { root: `../${slug}-seats`, branch: 'seat/{id}' } }));
    writeFileSync(path.join(baseline, '.agents/plan.yml'), plan(slug));
...
test('the machine cap bounds the sum of two projects; each project budget still bounds its own plan', (t) => {
  ...
  assert.match(m.cli(['plan', 'machine']).stdout, /cap       none/);
  const refused = m.cli(['plan', 'machine', '--parallel', '3']);
  assert.equal(refused.status, 1);
  assert.match(refused.stderr, /rerun with --apply/);
  assert.equal(m.cli(['plan', 'machine', '--parallel', '3', '--apply']).status, 0);
  assert.equal(JSON.parse(m.cli(['plan', 'machine', '--json']).stdout).parallel, 3);

  // alpha takes its own budget of two; beta gets the one slot the machine has left.
  assert.match(a.good(['plan', 'assign', '--apply']).stdout, /assigned 2\/2 · slots 2\/2/);
  const before = b.json(['plan', 'next']);
  ...
  assert.match(b.good(['plan', 'assign', '--apply']).stdout, /assigned 1\/1 · slots 1\/2/);
  assert.match(b.good(['plan', 'status']).stdout, /machine  cap 3 · 2 held by other projects/);
  assert.deepEqual(Object.keys(m.slots()).sort(), ['alpha/alpha-one', 'alpha/alpha-two', 'beta/beta-one']);
  assert.match(m.cli(['plan', 'machine']).stdout, /held      3\/3 managed seats/);

  // A done report frees the slot for whoever asks next.
  a.good(['seat', 'report', 'alpha-one', '--status', 'done']);
  assert.match(b.good(['plan', 'assign', '--apply']).stdout, /assigned 1\/1 · slots 2\/2/);
  assert.match(b.good(['plan', 'next']).stdout, /hold    beta-three  budget full \(2\)/);
  ...
  writeFileSync(path.join(a.baseline, '.agents/plan.yml'), plan('alpha').replace('parallel: 2', 'parallel: 4'));
  const capped = a.json(['plan', 'next']);
  assert.deepEqual(capped.next, []);
  assert.match(capped.held[0].reason, /machine cap full \(3; 2 held by other projects\)/);
  assert.equal(m.cli(['plan', 'machine', '--parallel', 'none', '--apply']).status, 0);
  assert.deepEqual(a.json(['plan', 'next']).next, ['alpha-three']);
});
...
test('two projects allocating at the same instant never share a slot', async (t) => {
  ...
  const a = m.project('alpha');
  const b = m.project('beta');
  assert.equal(m.cli(['plan', 'machine', '--parallel', '1', '--apply']).status, 0);
  for (let round = 0; round < 3; round += 1) {
    const results = await Promise.all([a, b].map((p) => new Promise((resolve) => {
      const child = spawn(process.execPath, [CLI, 'plan', 'assign', '--apply', '--project', p.baseline], { env: m.environment, stdio: ['ignore', 'pipe', 'pipe'] });
      let out = '';
    ...
    winner.good(['seat', 'report', holder.id, '--status', 'done']);
    winner.good(['seat', 'finish', holder.id, '--apply']);
  }
});
...
test('stale reservations are reclaimed without touching a live seat; unmanaged seats are shown, never counted', (t) => {
  const m = machine(t);
  const a = m.project('alpha');
  assert.equal(m.cli(['plan', 'machine', '--parallel', '2', '--apply']).status, 0);
  // A seat nobody reserved: planned by hand, outside Plan.
  a.good(['seat', 'plan', 'stray', '--task', 'by hand', '--apply']);
  ...
  assert.match(m.cli(['plan', 'machine']).stdout, /unmanaged 1 live seat no reservation names \(shown, not counted\)\n    alpha\/stray/);
  ...
  a.good(['seat', 'plan', 'alpha-one', '--task', 'by hand', '--apply']);
  a.good(['seat', 'finish', 'alpha-one', '--apply']);
  ...
  assert.equal(a.json(['seat', 'status']).seats.find((seat) => seat.id === 'stray').status, 'planned', 'the unmanaged seat is untouched');
  // A pure allocation sees the machine as loadPlan hands it in.
