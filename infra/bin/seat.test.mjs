...
import { parseSeatCliArgs, parseSeatProfile, readSeatFinished, readSeatProjectsIndex, readSeatState } from '../lib/seat.mjs';

const HERE = path.dirname(fileURLToPath(import.meta.url));
const CLI = path.join(HERE, 'cli.mjs');
const BACKEND = path.join(HERE, 'fixtures/process-overlay.mjs');
const SKILL = path.resolve(HERE, '../../skills/seat/SKILL.md');
const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));

function gitIn(cwd, args) {
  const result = spawnSync(
    'git',
    ['-c', 'core.hooksPath=/dev/null', '-c', 'commit.gpgSign=false', '-c', 'user.name=Seat Test', '-c', 'user.email=test@example.invalid', ...args],
    { cwd, encoding: 'utf8' }
  );
...
// A disposable baseline repo with Ground + Seat profiles. overlay: true wires
// the isolated process backend so `overlay create` really runs.
function fixture(t, { overlay = false, worktrees = true, createOn = null } = {}) {
  const root = realpathSync(mkdtempSync(path.join(tmpdir(), 'seat-')));
  const baseline = path.join(root, 'baseline');
  mkdirSync(path.join(baseline, '.agents'), { recursive: true });
  const stateDir = path.join(root, 'state');
  const environment = {
    ...process.env,
    GROUND_STATE_DIR: stateDir,
    GROUND_PROCESS_TEST_ROOT: root,
    GROUND_OVERLAY_VERIFY_TIMEOUT_MS: '3000',
    GROUND_OVERLAY_TIMEOUT_MS: '10000',
  };
  ...
  const runtime = { project: { slug: 'seat-test' }, services: { api: {} }, data: { infra: 'project' } };
  if (overlay) {
  ...
  const seat = { version: 1 };
  if (worktrees) seat.worktrees = { root: '../seats', branch: 'seat/{id}' };
  ...
  const run = (args, { cwd = baseline, env = {} } = {}) =>
    spawnSync(process.execPath, [CLI, 'seat', ...args], { cwd, env: { ...environment, ...env }, encoding: 'utf8', timeout: 20000 });
  ...
  const state = () => readSeatState('seat-test', environment).state;
  const stateFile = path.join(stateDir, 'seats', 'seat-test.yml');
  const indexFile = path.join(stateDir, 'seats', 'projects.yml');
  const groundStateFile = path.join(stateDir, 'seat-test.yml');
  ...
  return { root, baseline, head, environment, run, good, bad, state, stateFile, indexFile, groundStateFile, seatPath, images, gate, enter, launchOverlay };
}

test('seat profile parser accepts the documented shape and rejects unknown keys and placeholders', () => {
  const ok = parseSeatProfile('version: 1\nworktrees:\n  root: ../seats\n  branch: "seat/{id}"\n');
  assert.deepEqual(ok, { version: 1, project: null, worktrees: { root: '../seats', branch: 'seat/{id}' } });
  assert.deepEqual(parseSeatProfile(''), { version: 1, project: null, worktrees: null });
  const rejected = [
    'version: 2\n',
    'runners: {}\n',
    'worktrees:\n  root: ../seats\n  branch: "seat/{id}"\n  extra: 1\n',
  ...
  for (const text of rejected) {
    assert.throws(() => parseSeatProfile(text), /seat:/);
    count += 1;
  }
  assert.equal(count, rejected.length);
});

test('a project without Ground carries its slug in the Seat profile; with Ground the duplicate is rejected', (t) => {
  assert.deepEqual(parseSeatProfile('project: { slug: solo }\n'), { version: 1, project: { slug: 'solo' }, worktrees: null });
  assert.throws(() => parseSeatProfile('project: { slug: Solo Project }\n'), /DNS label/);
  assert.throws(() => parseSeatProfile('project: { slug: solo, namespace: x }\n'), /unknown key/);

  const root = realpathSync(mkdtempSync(path.join(tmpdir(), 'seat-solo-')));
  t.after(() => rmSync(root, { recursive: true, force: true }));
  const baseline = path.join(root, 'baseline');
  mkdirSync(path.join(baseline, '.agents'), { recursive: true });
  writeFileSync(path.join(baseline, '.agents/seat-profile.yml'), 'project: { slug: solo }\nworktrees: { root: ../seats, branch: "seat/{id}" }\n');
  ...
  const env = { ...process.env, GROUND_STATE_DIR: path.join(root, 'state') };
  delete env.SEAT_PROJECT;
  const run = (args) => spawnSync(process.execPath, [CLI, 'seat', ...args, '--project', baseline], { env, encoding: 'utf8' });
  ...
  t.diagnostic('Ground-less project: plan/status/finish 3/3; duplicate slug rejected 1/1');
});

test('the published example profile parses', () => {
  const example = readFileSync(path.join(HERE, '../../skills/seat/examples/seat-profile.yml'), 'utf8');
  assert.deepEqual(parseSeatProfile(example), { version: 1, project: null, worktrees: { root: '../acme-seats', branch: 'seat/{id}' } });
});

test('seat cli args enforce verb shapes', () => {
  assert.equal(parseSeatCliArgs([]).help, true);
  assert.throws(() => parseSeatCliArgs(['launch', 'w1']), /unknown command/);
  assert.throws(() => parseSeatCliArgs(['plan', 'w1']), /exactly one of --task or --task-file/);
  assert.throws(() => parseSeatCliArgs(['plan', 'w1', '--task', 'a', '--task-file', 'b']), /exactly one/);
  assert.throws(() => parseSeatCliArgs(['plan', 'W1', '--task', 'a']), /DNS label/);
  assert.throws(() => parseSeatCliArgs(['seat', 'w1', '--json', '--env']), /at most one/);
  assert.equal(parseSeatCliArgs(['seat', 'w1', '--task']).format, 'task');
  assert.equal(parseSeatCliArgs(['status', '--finished']).finished, true);
  assert.throws(() => parseSeatCliArgs(['finish', 'w1', '--finished']), /not valid for finish/);
  assert.throws(() => parseSeatCliArgs(['report', 'w1']), /requires --status/);
  assert.throws(() => parseSeatCliArgs(['report', 'w1', '--status', 'planned']), /--status must be one of/);
  assert.throws(() => parseSeatCliArgs(['finish', 'w1', '--force']), /not valid for finish/);
  assert.throws(() => parseSeatCliArgs(['status', 'w1', 'w2']), /at most 1/);
  const plan = parseSeatCliArgs(['plan', 'w1', '--task-file', 'tasks/w1.md', '--by', 'codex', '--apply']);
  assert.deepEqual([plan.verb, plan.id, plan.taskFile, plan.by, plan.apply], ['plan', 'w1', 'tasks/w1.md', 'codex', true]);
  assert.equal(parseSeatCliArgs(['seat', 'w1', '--shell']).format, 'shell');
  assert.equal(parseSeatCliArgs(['seat', 'w1']).format, 'text');
  assert.equal(parseSeatCliArgs(['projects', '--json']).json, true);
  assert.throws(() => parseSeatCliArgs(['projects', 'w1']), /no positional/);
  assert.throws(() => parseSeatCliArgs(['projects', '--project', 'x']), /not valid for projects/);
});
...
test('plan --apply records the baseline root in the machine index; finish leaves it alone; a moved root warns once', (t) => {
  ...
  const { index } = readSeatProjectsIndex(f.environment);
  assert.equal(index.version, 1);
  assert.equal(index.projects['seat-test'].root, f.baseline);
  assert.match(index.projects['seat-test'].updated_at, /^\d{4}-\d{2}-\d{2}T/);
  ...
  writeFileSync(f.indexFile, stringify({ version: 1, projects: { 'seat-test': { root: '/elsewhere/old', updated_at: '2026-01-01T00:00:00.000Z' } } }));
  ...
  assert.equal(readSeatProjectsIndex(f.environment).index.projects['seat-test'].root, f.baseline);
...
test('projects joins the index with live registries: two seats, then one finished; a missing index is projects 0', (t) => {
  ...
  const none = f.good(['projects'], { cwd: f.root, env: { GROUND_STATE_DIR: empty } });
  assert.match(none.stdout, /projects 0/);
  assert.deepEqual(JSON.parse(f.good(['projects', '--json'], { cwd: f.root, env: { GROUND_STATE_DIR: empty } }).stdout), { projects: [] });

  for (const id of ['w1', 'w2']) f.good(['plan', id, '--task', id, '--apply']);
  // projects is machine-wide: no seat profile above f.root, no --project.
  const two = JSON.parse(f.good(['projects', '--json'], { cwd: f.root }).stdout).projects;
  assert.equal(two.length, 1);
  assert.equal(two[0].slug, 'seat-test');
  ...
  assert.match(f.good(['projects'], { cwd: f.root }).stdout, /seat-test  .*  present  seats 2  finished 0  overlay off/);
  ...
  writeFileSync(f.indexFile, stringify({ version: 1, projects: { 'seat-test': { root: path.join(f.root, 'gone'), updated_at: '2026-01-01T00:00:00.000Z' } } }));
...
test('status --json marks each seat\'s overlay hostnames attached or not; without overlays the list is empty', (t) => {
  ...
  assert.deepEqual(seated.seats[0].hostnames, [{ host: 'api--w1.seat-test.localhost', service: 'api', attached: false }]);
  ...
  assert.deepEqual(live.seats[0].hostnames, [{ host: 'api--w1.seat-test.localhost', service: 'api', attached: true }]);

  // A pending env has no hostnames yet.
  f.bad(['plan', 'w2', '--task', 'pending', '--apply'], { env: { GROUND_PROCESS_TEST_ROOT: path.join(f.root, 'missing') } });
  const pending = JSON.parse(f.bad(['status', '--json']).stdout);
  assert.deepEqual(pending.seats.find((seat) => seat.id === 'w2').hostnames, []);
  assert.deepEqual(pending.seats.find((seat) => seat.id === 'w1').hostnames, [{ host: 'api--w1.seat-test.localhost', service: 'api', attached: true }]);

  const plain = fixture(t);
  plain.good(['plan', 'w1', '--task', 'no ground overlay', '--apply']);
...
test('a seat journals the state-changing catalog verbs it runs with their exit code, and never a read verb', (t) => {
  ...
  const seatEnv = { ...f.environment, SEAT_ID: 'w1', SEAT_PROJECT: f.baseline };
  ...
  catalog(['seat', 'status', '--json', '--project', f.baseline]);
  catalog(['seat', 'seat', 'w1', '--json', '--project', f.baseline]);
  assert.equal(events().length, 1, 'overlay status, urls, validate, seat status and seat seat record nothing');
  ...
  const archived = readSeatFinished('seat-test', f.environment).seats[0];
...
test('status lists every worktree of the baseline repository; one nobody seated has seat null', (t) => {
  ...
  assert.deepEqual([seated.path, seated.branch, seated.baseline], [f.seatPath('w1'), 'seat/w1', false]);
...
test('plan without --apply creates nothing; with --apply it creates a worktree seat that seat, report, status and finish round-trip', (t) => {
  ...
  assert.equal(gitIn(f.seatPath('w1'), ['symbolic-ref', '--short', 'HEAD']), 'seat/w1');
  ...
  const eventsFile = path.join(f.root, 'state/seats/events', 'seat-test', 'w1.events');
  assert.deepEqual(json.env_vars, { SEAT_ID: 'w1', SEAT_ENV: '', SEAT_BRANCH: 'seat/w1', SEAT_PROJECT: f.baseline, SEAT_SKILL: SKILL, SEAT_EVENTS: eventsFile, SEAT_EVIDENCE: path.join(path.dirname(eventsFile), 'w1.evidence.yml'), SEAT_CLAUDE_SETTINGS: path.join(path.dirname(eventsFile), 'w1.claude-settings.json') });
  assert.equal(json.evidence_file, json.env_vars.SEAT_EVIDENCE);
  ...
  const settings = JSON.parse(readFileSync(json.env_vars.SEAT_CLAUDE_SETTINGS, 'utf8'));
  ...
  assert.throws(() => parseSeatCliArgs(['diff']), /diff requires a seat id/);
  ...
  assert.ok(envLines.includes('SEAT_ID=w1'));
  ...
  const probe = spawnSync('sh', ['-c', `${shellLine} && pwd && env | grep '^SEAT_' | sort`], { encoding: 'utf8', env: f.environment });
  assert.equal(probe.status, 0, probe.stderr);
  const probeLines = probe.stdout.trim().split('\n');
  assert.equal(realpathSync(probeLines[0]), f.seatPath('w1'));
  assert.deepEqual(probeLines.slice(1), ['SEAT_BRANCH=seat/w1', `SEAT_CLAUDE_SETTINGS=${json.env_vars.SEAT_CLAUDE_SETTINGS}`, 'SEAT_ENV=', `SEAT_EVENTS=${eventsFile}`, `SEAT_EVIDENCE=${json.env_vars.SEAT_EVIDENCE}`, 'SEAT_ID=w1', `SEAT_PROJECT=${f.baseline}`, `SEAT_SKILL=${SKILL}`]);

  // A worker inside the worktree resolves the baseline through SEAT_PROJECT
  // (the worktree carries its own copy of .agents/).
  f.good(['report', 'w1', '--status', 'working', '--note', 'started'], { cwd: f.seatPath('w1'), env: { SEAT_PROJECT: f.baseline } });
  f.good(['report', 'w1', '--status', 'blocked', '--note', 'mock schema differs', '--session', 'sess-123'], { cwd: f.seatPath('w1'), env: { SEAT_PROJECT: f.baseline } });
  ...
  // One line more than the reports and finish steps alone: Seat's result line beside the done report.
  ...
  assert.match(finished.stdout, /branch    kept seat\/w1/);
  assert.equal(existsSync(f.seatPath('w1')), false);
  assert.equal(existsSync(f.stateFile), false);
  assert.equal(gitIn(f.baseline, ['rev-parse', '--verify', 'seat/w1']).length, 40, 'branch survives finish');

  // The journal outlives the seat: finish archives it for a later audit.
  const archived = readSeatFinished('seat-test', f.environment).seats;
...
test('with overlays, plan creates the env through Ground, a failed create stays pending and the same plan retries it', (t) => {
  const f = fixture(t, { overlay: true });
  const groundRegistry = () => parse(readFileSync(f.groundStateFile, 'utf8'));

  // Point the isolated backend at a root it does not own: Ground's create fails
  // after Seat has already made the worktree.
  const failed = f.bad(['plan', 'w3', '--task', 'retry me', '--apply'], { env: { GROUND_PROCESS_TEST_ROOT: path.join(f.root, 'missing') } });
  ...
  f.bad(['plan', 'w3', '--task', 'different task while pending', '--apply'], { env: { GROUND_PROCESS_TEST_ROOT: path.join(f.root, 'missing') } });
  ...
  assert.ok(groundRegistry().envs.w3, 'Ground tracks the env after retry');

  f.good(['plan', 'w1', '--task', 'real env', '--apply']);
  assert.equal(JSON.parse(f.good(['seat', 'w1', '--json']).stdout).env_vars.SEAT_ENV, 'w1');
  ...
  assert.equal(groundRegistry().envs.w3, undefined);
  assert.match(f.good(['status']).stdout, /envs       1\/1 tracked/);
  f.good(['finish', 'w1', '--apply']);
  assert.equal(existsSync(f.stateFile), false);
  assert.equal(existsSync(f.groundStateFile), false, 'Ground registry empties too');
...
test('two seats plan concurrently without clobbering each other', async (t) => {
  ...
  const launch = (id) =>
    new Promise((resolve) => {
      const child = spawn(process.execPath, [CLI, 'seat', 'plan', id, '--task', id, '--apply'], { cwd: f.baseline, env: f.environment, stdio: ['ignore', 'pipe', 'pipe'] });
      let stderr = '';
      child.stderr.on('data', (value) => { stderr += value; });
...
test('two seats attach concurrently; an in-flight attach is not a problem for status, a stalled one is', async (t) => {
  const f = fixture(t, { overlay: true });
  for (const id of ['w1', 'w2']) f.good(['plan', id, '--task', id, '--apply']);
  const worker = (id) => ({ cwd: f.seatPath(id), env: { SEAT_PROJECT: f.baseline } });
  ...
  const ground = parse(readFileSync(f.groundStateFile, 'utf8'));
  assert.equal(ground.envs.w1.services.api.image, f.images[0]);
  assert.equal(ground.envs.w2.services.api.image, f.images[1]);
...
// the task through SEAT_* alone, then report its own completion.
test('a scripted worker seated through --shell reads seat, skill and task, commits on its branch, and reports done', (t) => {
  const f = fixture(t);
  const worker = path.join(HERE, 'fixtures/seat-worker.mjs');
  ...
  assert.equal(readFileSync(path.join(f.seatPath('w1'), 'WORK.md'), 'utf8'), 'seat w1 on seat/w1: write the work note\n');
  const seat = f.state().seats.w1;
  assert.equal(seat.status, 'done');
  assert.match(seat.session, /^worker-\d+$/);
  // The done report is the worker's line; the git facts of its result are Seat's line beside it.
...
test('with create_on: attach, plan creates no env, status shows unattached without a problem, and the seat attach creates it', (t) => {
  ...
  const groundRegistry = () => (existsSync(f.groundStateFile) ? parse(readFileSync(f.groundStateFile, 'utf8')) : null);
  const planned = f.good(['plan', 'w1', '--task', 'deferred env', '--apply']);
  assert.match(planned.stdout, /env       1\/1 w1/);
  assert.equal(groundRegistry(), null, 'no overlay create at plan');
  ...
  assert.equal(groundRegistry().envs.w1.worktree, f.seatPath('w1'), 'the env is owned by the seat worktree, not the baseline');
  const after = f.good(['status']);
