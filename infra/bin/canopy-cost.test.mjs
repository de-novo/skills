...
// read per project and no second Ground probe, because Seat's status
...
function fixture(t) {
  ...
  const projects = [false, true].map((overlay, index) => {
    ...
    run(['seat', 'plan', 'w1', '--task', 'Counted', '--by', 'worker', '--apply', '--project', baseline]);
  ...
  // Seat writes them, so the newest-first tail and the total can be read.
  const record = (n) => ({ id: `old-${n}`, finished_at: `2026-09-0${1 + (n % 9)}T00:00:${String(n).padStart(2, '0')}.000Z`, worktree: `/gone/${n}`, owned: true, branch: `seat/old-${n}`, base: 'a'.repeat(40), task: `old ${n}`, env: null, by: 'worker', session: null, created_at: '2026-09-01T00:00:00.000Z', status: 'done', journal: [{ at: '2026-09-01T00:00:00.000Z', actor: 'seat', event: 'report', detail: `done: old ${n}` }] });
  writeFileSync(path.join(root, 'state/seats/cost-0.finished.yml'), stringify({ version: 1, project: 'cost-0', seats: Array.from({ length: 25 }, (_, n) => record(n)) }));
  environment.CANOPY_TEST_PROJECTS = JSON.stringify(projects);
  return { root, projects, environment, projectsCli: PROJECTS, run };
}

test('a full view costs one status and one bounded archive read per project; Ground is not asked twice', async (t) => {
  const f = fixture(t);
  const trace = [];
  const state = await collectState({ ...f, trace });
  assert.equal(state.projects.length, 2);
  const calls = trace.map((line) => line.split(' --project ')[0]);
  assert.deepEqual(calls.filter((line) => line.startsWith('overlay status')), [], 'no second Ground probe: seat status carries the report');
  assert.equal(calls.filter((line) => line === 'seat status --json').length, 2);
  assert.equal(calls.filter((line) => line === 'seat status --finished --json --tail 20').length, 2);
  assert.equal(trace.length, 5, 'discovery + two per project');
  const overlayProject = state.projects.find((p) => p.slug === 'cost-1');
  assert.equal(overlayProject.ground.observed_by, 'seat status');
  assert.equal(overlayProject.ground.counts.environments, 1);
  assert.equal(overlayProject.ground.project_status.ok, true);
...
test('a detail page reads one seat of one project, not every project on the machine', async (t) => {
  ...
  assert.deepEqual(trace.map((line) => line.split(' --project ')[0]), ['seat projects --json', 'seat status w1 --json']);
  ...
  assert.deepEqual(pageTrace.map((line) => line.split(' --project ')[0]), ['seat projects --json', 'seat status w1 --json', 'seat diff w1'], 'discovery, one seat, one diff');
  pageTrace.length = 0;
  assert.equal((await fetch(url + '/chat/cost-0/w1')).status, 404);
  assert.deepEqual(pageTrace.map((line) => line.split(' --project ')[0]), ['seat projects --json', 'seat status w1 --json']);
});
