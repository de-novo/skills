# Facts — the values, the log, the envelope, the transitions, the query, the CLI

The long facts of the assertion log. Names: [glossary](../../../docs/glossary.md). Pattern: [SKILL.md](../SKILL.md). The human
page: [README.md](../README.md). Reasons:
[`docs/facts-design.md`](../../../docs/facts-design.md).
...
seat report --status done ──▶ propose --from-seat <id>   (the Plan seam, read-only)
```

## Values

`.agents/facts.yml`, tracked. The project's vocabulary.
...
1. `--by <id>`.
2. `seat:<SEAT_ID>`, when the environment carries it.
3. `seat:<id>` for the seat whose worktree holds the current directory,
   read from the Seat registry. A worker standing in its own worktree
   needs nothing else.
...
shared machine that trust is the same trust Seat's registry already
...
0), taken from the Seat registry and never typed by hand. `ref` is
...
Inside a playground sandbox the catalog's guard strips every `SEAT_*`
variable from a verb aimed at the sandbox; a seat there is still found
by its worktree. Measured 2026-09-08:
[evidence](../../../docs/evidence/2026-09-08-facts-sandbox.md).

## Log

`<state>/facts/<slug>.jsonl`. `<state>` is `GROUND_STATE_DIR` when set,
otherwise `~/.dev-infra`, the same root Seat's registry lives under; the
slug is the project's Seat slug. One JSON object per line, append-only:
...
Every write takes the same file lock Seat's registry uses (`<log>.lock`,
...
| `agent_id` | Who proposed it | `--by`, or `seat:<SEAT_ID>` |
...
- A seat's heartbeat; Seat's journal has it. Point at the seat instead.
...
list line per row, in the shape a seat brief takes.
...
`de-novo skills facts <verb>`. Every verb takes `--project ROOT`;
without it the Seat profile above the current directory names the project.
...
reading line. A project without `.agents/facts.yml` gets none; a saved
...
[`docs/facts-cases.md`](../../../docs/facts-cases.md) says what each
round measured.
