...
name: facts
description: >-
  de-novo Facts — assertions, separate from Plan. One append-only log of
  ...
  /facts. Values live in .agents/facts.yml (domains, entity types,
...
the CLI are in [references/log.md](references/log.md). Plan pattern:
[plan](../plan/SKILL.md). Seats: [seat](../seat/SKILL.md).

## Two graphs, one seam

| Graph | Node | Owner | Home |
| --- | --- | --- | --- |
| Plan | an item of work and its state | the plan | `.agents/plan.yml` |
| Facts | an assertion and its status | the log | `<state>/facts/<slug>.jsonl` |
...
latter). Facts never changes the plan, never assigns, never marks an
item done.
...
2. **Propose it in the envelope.** Subject, predicate, object, the subject's
   ...
   `.agents/facts.yml`; if the vocabulary lacks one, say so to a person
...
- **Facts does not write the plan.** The Plan seam reads seats; it does not touch them.

## Not this skill

- Deciding the work or its order. Plan.
- Seating, reporting, or finishing a worker. Seat.
