...
seat report --status done|blocked ──▶ propose --from-seat <id>   (the Plan seam, read-only)
```
...
predicates) is declared in `.agents/facts.yml`; a missing word is a
...
Beside Plan, not inside it: two graphs, one read-only seam
...
| --- | --- | --- | --- |
| One append-only log per project under `~/.dev-infra/facts/`, and a lock file beside it while a write is in flight | Nothing | Nothing; it reads Seat's registry for `--from-seat` | Nothing is deleted; a wrong fact is amended or invalidated with a reason |

## Apply to a project

1. Write `.agents/facts.yml` with the domains and the first types.
   Copy [examples/facts.yml](examples/facts.yml) and cut it down.
2. Load the skill through `.agents/skills/facts` the way the project
   loads the others.
3. Run `facts status`. It prints the log path and zero counts.
...
Reasons: [`docs/facts-design.md`](../../docs/facts-design.md). Cases:
[`docs/facts-cases.md`](../../docs/facts-cases.md).
