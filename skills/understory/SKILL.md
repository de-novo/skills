...
[README.md](README.md). The graph itself is Forester's:
[forester](../forester/SKILL.md).

## The one rule

**A map points; it does not copy.** Every fact in the document already has
a home: the plan file, the Forester README, the design note, an evidence
...
2. Write the sections in the order the README gives, each as short as it
...
5. End with the pointers: the plan, the Forester README, the design note,
