...
Naming rule: [glossary](../../docs/glossary.md).

```
goal ──(grill, then an agent writes)──▶ .agents/plan.yml
                                              │
...
edges; then assigns the ready ones through Seat, up to the budget. The
...
You type `/plan`; the agent does not reach for it on its own. Reach for
...
seat with a task you already have, use [seat](../seat/README.md) directly.

## It's working if

- `plan plan` prints a graph a person reads and understands without asking you.
...
The first step after a goal: it feeds Seat, Understory draws its graph,
Facts keeps the decisions the grilling locked. Root map:
...
| --- | --- | --- | --- |
| Seats through Seat; a snapshot, per-session events files, and machine slot reservations under `~/.dev-infra/plans/`; for Claude Code a per-session settings file, and the seat path's trust entry in `~/.claude.json` only when the local file sets `pretrust_worktrees`; with `hooks --apply` only, one marked entry per event in the global hook stores of Codex, Grok, Cursor, and OpenCode, each gated to fire only when `PLAN_EVENTS` names a file under that state directory, plus one marked trust table per Codex entry in its `config.toml` | `@lydell/node-pty`, an optional npm dependency, only for `serve` | The tool command templates in `plan.local.yml`, in a pseudo-terminal, with the task line passed as one argv element and no shell in between | `hooks --remove --apply` takes back exactly the marked entries; Ctrl-C on `serve` closes every session and removes its snapshot and socket |
...
4. `plan plan`, read it, then either `plan assign --apply` and seat
   ...
   `plan attach <id>` whenever `status` shows a session that needs
...
[SKILL.md](SKILL.md). Reasons: [`docs/plan-design.md`](../../docs/plan-design.md).
