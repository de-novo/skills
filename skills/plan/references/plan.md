# Plan — the plan, the budget, the allocator

The long facts of Plan. Names: [glossary](../../../docs/glossary.md). The fields, the allocation rule, the CLI, the
sessions. Pattern: [SKILL.md](../SKILL.md). The human page:
[README.md](../README.md). Reasons: [`docs/plan-design.md`](../../../docs/plan-design.md).

```
goal ──(an agent reading the skill)──▶ .agents/plan.yml
                                              │
             .agents/plan.local.yml ──▶ allocate ──▶ seat plan <id> --apply
...
| Field | Meaning |
| --- | --- |
| `tasks.<id>` | The item id. It becomes the seat id, so it is a DNS label of at most 63 characters |
...
commit. `plan plan --json` prints it per item; `assign` and `serve`
hand it to Seat as the seat's `revision`. A seat, live or archived, counts
...
afterwards with `seat seat <id> --task`: the title, the revision, the seat
id and evidence file as literal values (a launcher may stop on a
`$VARIABLE`), the base commit, the scope (`edit only …`, `read-only`, or
`unchecked`), each
...
evidence file at `$SEAT_EVIDENCE`, commit, report done, one command per
line), and the brief's text under its path and digest. Seat also records the scope
(`--owns` or `--read-only`), the revision, and the dependency inputs on the
seat.

## Local file

`.agents/plan.local.yml`, gitignored by the catalog's `*.local.yml` rule.
...
Computed from the plan, Seat's live registry, its finished archive, and
...
A **result** is what Seat records at `report --status done`: the seat's
...
branch (ancestry proves it), or, after a squash or rebase, records `seat
integrate <id> --commit <sha> --apply` and that commit's ancestry proves
it. Plan never merges. `verification` per item is `unverified` (done
...
directory) are **unmanaged**: `plan machine` lists them and counts
nothing for them. The cap governs what Plan seats, not what else
runs.
...
```text
de-novo skills plan plan   [--json] [--project ROOT]
de-novo skills plan next   [--json] [--project ROOT]
de-novo skills plan assign [--apply] [--json] [--project ROOT]
de-novo skills plan status [--json | --watch] [--project ROOT]
de-novo skills plan serve  [--project ROOT]
de-novo skills plan attach ID [--project ROOT]
de-novo skills plan restart ID [--project ROOT]
de-novo skills plan hooks  [--apply | --remove --apply] [--json]
de-novo skills plan machine [--json | --parallel N|none --apply]
```
...
| assign | same as next | runs `seat plan <id> --task <handoff> --by plan --revision <rev> [--owns … | --read-only] [--input <dep>=<sha> …] --apply` for each chosen item; prints `assigned k/n · slots a/parallel`; non-zero if any seat failed to plan |
...
| serve | foreground daemon: every poll it does what `assign --apply` does, starts the tool of each active item as a real interactive session in a pseudo-terminal it holds, reads the tool's hook events, closes a session whose seat reported done or was finished by a person, drops the record of a session whose seat was replaced by a new attempt so the new seat gets its own launch, and serves `attach` and `restart`. Before a launch it checks the tool is declared and on PATH, the worktree exists, and the seat's env is not pending; a failed check is a `failed` session naming its kind and nothing is spawned. A seat whose env is pending is planned again with a doubling wait (capped at a minute) up to five times, then marked `failed` (`env-pending`). Ctrl-C closes every session and removes its snapshot, socket, and lock. One serve per project: the second start stops at the lock (`<snapshot>.serve.lock`, reclaimed only from a dead pid) and removes nothing; a socket that answers is never unlinked. Started after a crash, it says which sessions the previous daemon held and launches them again as fresh contexts, never as native resumes | — |
...
`serve` records a snapshot at `~/.dev-infra/plans/<slug>.yml`
(`GROUND_STATE_DIR/plans/` under the override) with its pid, its socket,
...
| `exited` | The process ended on its own; `exit` carries the code, and `note` says `plan restart` launches it again |
...
Each session's events go to the seat's own file, `SEAT_EVENTS`, laid by Seat at `plan --apply` and read back by `seat status`, so a session serve holds and one another launcher started look the same. Claude Code sessions get their hooks through `--settings <file>`, the seat's `SEAT_CLAUDE_SETTINGS`, never into the worktree or the person's own settings.
...
on the trust dialog shows as `needs-input`, and `plan attach <id>`
...
`PLAN_EVENTS`: outside a serve session the hook consumes its input and
...
| codex | `$CODEX_HOME/hooks.json`, else `~/.codex/hooks.json`, with the trust tables in `config.toml` beside it | UserPromptSubmit, PreToolUse, PostToolUse, PermissionRequest, Stop | none: `hooks --apply` records the trust Codex checks (the hash of the handler's normalized identity, as `codex-rs` computes it, over a group written without a matcher key, which Codex would otherwise hash in for the tool events); without that record Codex runs nothing and says nothing in the events file, and the TUI parks on "Hooks need review". Its directory-trust prompt is a person's answer, shown as needs-input by the screen fallback |
| grok | `~/.grok/hooks/de-novo-plan.json` (a file of its own) | the Claude set | none seen |
...
| opencode | `$OPENCODE_CONFIG_DIR/plugins/de-novo-plan.js`, else `~/.config/opencode/plugins/` | a plugin mapping session status and permission events to the same names | none seen |

Each session runs in the seat's worktree with `SEAT_ID`, `SEAT_ENV`,
`SEAT_BRANCH`, `SEAT_PROJECT`, `SEAT_SKILL`, `PLAN_SEAT`, and
`PLAN_EVENTS`.

`--project ROOT` names the baseline checkout, as for Seat; omitted, Plan
uses `SEAT_PROJECT`, then the nearest `.agents/seat-profile.yml` above the
cwd. A Seat profile is required because the seats are Seat's.
...
and `integration` are the done seat's records as Seat keeps them.
