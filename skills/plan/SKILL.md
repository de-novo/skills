...
name: plan
description: >-
  de-novo Plan — break a body of work into a plan of items with
  dependencies and file claims, set how many may run at once on this
  machine, and keep that many seated through Seat. Values live in
  .agents/plan.yml (the plan, tracked) and
  .agents/plan.local.yml (this machine's budget, never committed). The
...
The allocator is a pure function of its inputs: the plan, the seats,
...
in [references/plan.md](references/plan.md). Seat pattern: [seat](../seat/SKILL.md).
...
worth keeping: propose it to Facts as a `decision` when the project has
the log (Call the Skill tool with "facts").
...
1. **Split it into items a single worker can finish alone**, each with a
   ...
   forbidden: the shape in [the seat brief](../seat/references/seats.md#writing-a-seat-brief)).
...
3. **Declare `depends_on`** only where the second item cannot start until
   ...
   `seat integrate`). `{ item: <id>, needs: order }` means only that the
...
Then write `.agents/plan.yml`, validate it with
`de-novo skills plan plan`, and show the person the printed graph before
...
```bash
de-novo skills plan plan             # every item and why it is in its state
de-novo skills plan next             # what assign would seat now; changes nothing
de-novo skills plan assign --apply   # seat the chosen items through Seat
de-novo skills plan status           # slots filled, items waiting
```

`assign --apply` creates one seat per chosen item, with the item id as
...
Seat's rules apply: the worker reports its own state, and `report --status
...
`plan serve`, run in a terminal of its own, does the refill by itself
...
and `plan restart <id>` launches a failed or exited item again once
you have fixed it. One serve runs per project. `plan attach <id>`
...
Run `plan hooks --apply` once on a machine so Codex, Grok, Cursor, and
...
- **Plan creates no environment or worktree itself.** It asks Seat,
  which asks Ground.
...
- A screen. Canopy shows seats; `plan plan --json` is shaped for it to read.
