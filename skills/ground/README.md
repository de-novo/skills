...
Shared local base and thin overlays for many projects and agents on one machine. Naming rule: [glossary](../../docs/glossary.md).
...
|       +------------------------------------------------------------+       |
|       | de-novo skills urls prints them — Ground starts no listener |       |
...
- Overlay only the apps you changed. Unattached `{app}--{env}` hostnames fall through to that project's baseline — when the project has a listener. Ground prints the names; it does not route packets.
- One engine set on the machine. Isolate by database and prefix (`acme_` / `sideapp_`), not by port.

```
  Ground CLI checkout                 consuming project
  -----------------                  -----------------
...
The base. Seat stands on it; Plan never touches it directly;
