...
`runtime.commands.overlay` is the one command Ground dispatches to put a
changed service beside a project's baseline. The project writes it; Ground
never invents it.
...
and prints one JSON object as its last stdout line. Ground tokenizes the
...
- Report the image you observed, in the same canonical form attach was given.
  Ground compares the two exactly and resolves nothing. During a replacement,
...
were handed — environment, service, image — verbatim; Ground rejects a receipt
...
- **Refused.** Anything you will not honor — a service this project does not
  ...
  Ground withdraw the journal it wrote and leave the environment usable. A
...
The Ground catalog carries a small process backend used to execute this
contract for real:
...
and Ground creates from the caller's directory, so verify run from somewhere
...
skipped rather than guessed. `overlay.attachable` is enforced by Ground's front
