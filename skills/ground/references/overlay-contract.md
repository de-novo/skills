...
Ground owns the lifecycle gate, lease registry, and counted receipts. The
consuming project owns its overlay workloads and routing implementation through
`runtime.commands.overlay`. Ground never invents a Docker, Compose, or Kubernetes
cleanup command.
...
Use only the Ground front door:
...
lease registry, so Ground cannot distinguish a live environment from leaked
workloads.
...
For every applied mutation, Ground writes a pending operation for that
...
`--apply`; Ground redispatches the idempotent project command, checks status
...
exit code as the text report. Tools built on Ground read this, not the text.
...
Ground records `created_at` and `last_used_at` for every environment. Successful
...
`untracked` drift. Ground does not assign an invented age and will not prune it
...
`overlay.plan_first` tells Ground how the project command behaves:

| Profile value | Ground command without `--apply` | Ground command with `--apply` |
| --- | --- | --- |
...
Project-specific arguments may follow `--`. Ground never retries by guessing
from stderr.

## Project command invocation

Ground tokenizes `runtime.commands.overlay` as an executable and arguments. It
...
env     = the caller's environment + GROUND_CALLER_CWD (the directory the Ground
          command was run from, resolved; a seat worktree when a worker calls)
```

`GROUND_CALLER_CWD` lets an adapter default a per-environment worktree without a
passthrough flag. It is informational; an adapter that needs the worktree must
still validate it belongs to the repository.

`GROUND_OVERLAY_TIMEOUT_MS` may set a positive timeout in milliseconds
(`DEVINFRA_OVERLAY_TIMEOUT_MS` is a legacy alias read only when the Ground name
is unset). Postcondition polling also defaults to 120000ms;
`GROUND_OVERLAY_VERIFY_TIMEOUT_MS` may set that deadline. Each internal status
...
as that line; Ground then withdraws the pending journal it wrote for that
...
Ground still does not start a hostname listener; that proxy value is declared
intent.
...
image reference in attach and status; Ground compares it exactly and does not
...
Ground's registry or an optimistic mutation result. Keep an environment present
...
`GROUND_STATE_DIR` overrides its directory for isolated tooling and tests.
...
backend updates such as a shared routing file. Ground's registry lock does not
...
Overlay cleanup never stops Ground's shared engines. There is no `down` command.
