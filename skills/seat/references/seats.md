# Seat — profile, registry, CLI, Canopy, the seat brief

The long facts of Seat. Names: [glossary](../../../docs/glossary.md). Pattern: [SKILL.md](../SKILL.md). The human page:
[README.md](../README.md).

```
  human decides                    Seat owns                     Ground owns
  -------------                    ----------                     ----------
  ...
  when to merge                      envs, reported states
     ...
     plan ──► seat ──► (launcher, not Seat) ──► worker reports ──► finish
```


## Profile

`.agents/seat-profile.yml` in the consuming project, next to Ground's
`runtime-profile.yml`. Allowed top-level keys: `version`, `project`,
`worktrees`.

```yaml
version: 1                       # omitted = 1; other values rejected
# project: { slug: myproject }   # only for a project WITHOUT Ground; with a
...
worktrees:                       # omit when every seat adopts an existing worktree
  ...
  branch: "seat/{id}"           # only {id} is substituted
```

With Ground, Seat reads the project slug and overlay mode from
`runtime-profile.yml`. Without Ground there are no overlay envs; seats are
worktrees and reports only.
There is no launcher configuration: which tool runs in a seat is the
launcher's own setting, not a Seat value. Example: [examples/](../examples/).

## Registry

`~/.dev-infra/seats/<slug>.yml` (`GROUND_STATE_DIR/seats/` when the override
is set). One record per seat: worktree, `owned` (created by Seat or
...
planner's label for which version of the task this seat is for; Seat
...
append-only `journal` of what Seat did and what the worker reported. The
...
Journal entries are `{ at, actor, event, detail }`. `actor` is `seat` for
...
a process with `SEAT_ID` set runs a state-changing catalog verb — `overlay
...
registry, finished seats from the archive, and whether the project's Ground
profile has overlays on.

## CLI

```text
de-novo skills seat plan   ID (--task TEXT | --task-file PATH) [--worktree PATH | --resume]
                            [--owns CLAIM ... | --read-only] [--revision TEXT] [--input ID=SHA ...] [--by LABEL] [--apply]
de-novo skills seat seat   ID [--json | --env | --shell | --task]
de-novo skills seat report ID --status working|blocked|done [--note TEXT] [--session REF]
                            [--evidence PATH|-] [--accept-outside-scope]
de-novo skills seat integrate ID --commit SHA [--by LABEL] [--note TEXT] [--apply]
de-novo skills seat status [ID] [--json] [--finished]
de-novo skills seat diff   ID [--json]
...
Ground already know all of it:

| Field | Where | Shape |
| --- | --- | --- |
| `hostnames` | per seat | `[{ host, service, attached }]` — the overlay hostnames of the seat's env, read from Ground's `urls --env <id> --json` (Seat renders no hostname itself). A project renders a hostname for every service; `attached` says which of them the overlay status inventory actually holds. Empty without a Ground profile, with overlays off, or while the env is pending; `attached` is `false` when the env is not tracked |
...
| `worktrees` | project | `[{ path, branch, head, seat, baseline }]` from the baseline's `git worktree list --porcelain`. `seat` is the seat id holding that path, or `null` — somebody works in parallel and Seat does not know it. `changes` is computed for seats only: reading another person's worktree is not Seat's business |
...
`status --json` also carries `overlay_report`: Ground's `overlay status
--json` report as Seat read it for this status, or `null` when overlays
are off or no seat wanted an env, so a reader that needs Ground's view
takes Seat's one observation instead of probing again.
...
`--project ROOT` names the baseline checkout. Omitted, Seat uses
`SEAT_PROJECT`, then the nearest `.agents/seat-profile.yml` above the cwd.
A worker inside a worktree must rely on `SEAT_PROJECT`, because the worktree
carries its own copy of `.agents/`.

Seat environment: `SEAT_ID`, `SEAT_ENV` (empty without overlays),
`SEAT_BRANCH`, `SEAT_PROJECT`, `SEAT_SKILL` (the path to this skill's
SKILL.md inside the installed catalog, so the project need not vendor or
symlink it), `SEAT_EVENTS` (the seat's events file, which the agent
tools' hooks append to), `SEAT_EVIDENCE` (`<id>.evidence.yml` beside it:
...
it would fall outside the scope), and `SEAT_CLAUDE_SETTINGS` (a hooks
file a Claude Code launcher passes as `--settings`). `plan --apply` lays
the events and settings files under `<state>/seats/events/<slug>/`; the
worker writes the evidence file; `finish` removes all three. `seat --json`
names the evidence file as `evidence_file`. Any
launcher that starts a tool with this environment, and `plan hooks
...
It reads only public CLI JSON: `seat projects --json`, then per project
`seat status --json` and `seat status --finished --json --tail 20`. The
Ground report comes from the `overlay_report` that Seat's status carries
(the one observation Seat made, marked `observed_by: seat status`);
`overlay status --json` is asked only when a Seat old enough not to
...
Each project shows its counts, Ground report and problems, then one card
...
`seat diff --json` reports it, one fold per file, additions and deletions
...
`seat status <id> --json`), never every project on the machine, and
...
Inspect live seats with `de-novo skills seat status --json` and read their
briefs with `de-novo skills seat seat <id> --task`. Compare intended scope,
...
- **Verification:** name the project's check or give its existing command and
  ...
  worker can run it alone with the seat's available resources. With no Ground
...
use `--task` for a short brief; give the boundary to Seat as well with
`--owns` (or `--read-only`), so the done report is held against it. Under
Plan the brief is the item's `brief` file and the handoff is generated.
...
eval "$(de-novo skills seat seat w1 --shell)" && <your agent cli>

# a launcher that makes its own worktrees: adopt it
de-novo skills seat plan w2 --worktree <path-the-launcher-made> --task-file tasks/w2.md --by codex --apply

# a script
de-novo skills seat seat w3 --json | my-launcher --stdin

# an agent CLI that takes the first prompt as an argument
eval "$(de-novo skills seat seat w1 --shell)" && <agent> "$(de-novo skills seat seat w1 --task)"
```
...
launcher answers that, not Seat.
