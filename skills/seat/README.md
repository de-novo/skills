# Seat

One agent seat on Ground. A seat is a worktree, an overlay env
when the project has one, a task, and a journal of what the worker
reported. Seat launches no agent: the person picks the launcher and who
sits where. Naming rule: [glossary](../../docs/glossary.md).

```
  human decides                    Seat owns                     Ground owns
  -------------                    ----------                     ----------
  which task → which seat          worktree per seat              overlay env + lease
  which launcher (terminal,        registry + journal             names, engines
    worktree app, tmux, ACP)       counts: seats, worktrees,      readiness verdict
  when to merge                      envs, reported states
                                   seat handoff (--shell/--json)

     plan ──► seat ──► (launcher, not Seat) ──► worker reports ──► finish
```

## What it does

Prepares a seat, hands it to whatever launcher you use, counts what the
workers report, and clears the seat when a person says the work is merged.
The defining constraint: completion is the worker's own `report --status
done`, never a process exit, and Seat never repairs runtime state silently.

## When to reach for it

A seat reaches for it on its own when it wakes with `SEAT_ID` set. A person
reaches for it to seat parallel work across worktrees, or to count who sits
where. For deciding *what* the work is and how many run at once, use
[plan](../plan/README.md).

## It's working if

- `seat status` exits zero and its counts match what you see in `git worktree list`.
- Every live seat has a task a stranger could start from, a scope its done report is held against, and its last journal line says what the worker did.
- Nobody merges, pushes, or runs `finish` from inside a seat.

## Where it fits

Plan assigns into it; Ground stands under it; Facts reads its done
and blocked reports; Canopy (`de-novo skills canopy`) is the live screen
over its JSON. Root map: [How the skills fit](../../README.md#how-the-skills-fit).

## What it does to your machine

| Writes | Downloads | Runs | Undo |
| --- | --- | --- | --- |
| A git worktree and branch per seat at `worktrees.root`; the registry and journal under `~/.dev-infra/seats/`; one journal line per state-changing catalog verb a seat runs; per seat, an events file the agent tools' hooks append to and a Claude Code settings file, under `~/.dev-infra/seats/events/` | Nothing | `git worktree add`, and Ground's overlay verbs when the project has overlays; Canopy reads, never copies, the session transcript a seat's tool names (Claude Code's `~/.claude/projects/…`) for its chat page | `finish --apply` removes a clean Seat-made worktree, the two seat files, and archives the journal; branches are kept |

## Apply to a project

1. Plant Ground first ([ground planting](../ground/references/planting.md#apply-to-a-project)).
   Seat needs `project.slug` from `runtime-profile.yml`.
2. Write `.agents/seat-profile.yml` with a `worktrees` section, or none if
   every seat will be adopted.
3. Symlink the skill: keep the canonical copy under `.agents/skills/seat`
   and symlink tool-specific dirs to it. Never copy the body.
4. `de-novo skills seat plan <id> --task … ` to see the plan, then `--apply`.

Design record and the reasons behind "no launcher": [docs/seat-design.md](../../docs/seat-design.md).
Contributing: [root AGENTS.md](../../AGENTS.md).

## Pointers

Fields, registry, CLI, Canopy, the seat brief, launcher handoff:
[references/seats.md](references/seats.md). Pattern: [SKILL.md](SKILL.md).
