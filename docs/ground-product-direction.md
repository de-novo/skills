# Ground's value, its competitive position, and the experience it intends to give
...
[the skill](../skills/ground/SKILL.md),
[the profile](../skills/ground/references/runtime-profile.md), and
[the overlay contract](../skills/ground/references/overlay-contract.md).

## The judgment

**What Ground publishes is a shared contract, and a reproducible verification
...
The value Ground should focus on is this: the rules for using the environment do
...
| Who, and in what situation | The repeated cost | What Ground aims to reduce | Fit judgment |
| --- | --- | --- | --- |
...
material and on Ground's current implementation. It is also not a claim that the
same contract could not be implemented on another tool.

| Alternative | The experience found in the official documentation | What it implies for Ground |
| --- | --- | --- |
...
| OrbStack | Automatic domains for containers and Compose services, web port detection, and HTTPS. [Official documentation](https://docs.orbstack.dev/docker/domains) | Reaching things by name already exists. Ground, which prints URLs, cannot claim an advantage on address convenience alone |
...
| A team's existing project scripts and operating documents | The alternative each team already has. The concrete cost and quality have to be measured per project | This is the most important thing to compare against. If Ground does the same job with more configuration, it is better not to adopt it |
...
writer ownership and the ban on shared DB access are largely procedure. Ground is
...
[the shared evaluation procedure](evaluation/ground-pilot.md). A consuming
...
1. Choose two real consuming projects with different operating styles. At least
   ...
   the same time.
2. Perform similar work under the existing procedure and under the Ground
...
> Ground helps several projects and agents handle running, confirming,
