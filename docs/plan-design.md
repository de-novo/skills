# Plan — analyse the work, then keep this machine's slots full

Written 2026-09-08. Status: design proposal. The name was chosen the same day:
a plan plans the forest, decides what is planted where, and knows how many
trees the ground can carry, which is this skill's whole job next to Ground (the
ground), Seat (the seats), and Canopy (the screen). Once the implementation lands,
`skills/plan/SKILL.md` owns the pattern and `infra/lib/plan.mjs` owns the
...
3. **Allocate.** Pick the ready items, up to the budget, and hand them to Seat
   as seats. When a worker reports done, a slot frees and the next item can go.

## Why it is not part of Seat

Seat seats one worker: a worktree, an environment, a task, a journal. It has
...
thing. Seat's contract says it launches no agent. Plan is free to decide
its own contract without touching that.

This is the same split that already happened once: Ground stops at the ground,
Seat seats workers on it.

## Where the values live

The catalog rule is that a skill is a pattern and the values live in the
project. Plan splits its values one step further, because two of them are
not the same kind of fact.

| Fact | Home | Tracked |
| --- | --- | --- |
| The pattern: how to analyse work into a plan | `skills/plan/SKILL.md` | yes |
| This project's plan, and a budget if the project sets one | `.agents/plan.yml` | yes |
| This machine's budget, used when the project sets none | `.agents/plan.local.yml` | **no** |
...
its claim, and Seat's existing `overlaps` still reports what actually happened.
...
```text
plan plan    [--json]     the graph: done, active, ready, blocked, and why
plan next    [--json]     what would be assigned now; changes nothing
plan assign  --apply      create the Seat seats for those items
plan status  [--json]     the budget: slots filled, items waiting
```

`next` exists so the decision can be read before it is taken, the same way
Ground's overlay verbs plan before they apply.
...
- **No model call in the CLI.** The analysis is done by an agent reading the
  skill, and it produces a file. `infra/lib/plan.mjs` parses and decides.
...
- **Plan does not create environments or worktrees itself.** It asks Seat,
  which asks Ground. One backend, named once.
...
- A screen. Canopy already owns that, and `plan plan --json` should be
  shaped so Canopy can read it.

## Open question

**Does Plan launch the agent into the seat it created?** Today a seat is
created and a person or their tool starts the worker. Plan could do that
...
- A fresh directory that is its own git root parks Claude on the trust dialog
  ...
  (the CLIs treat the repo root as the trust domain), so Seat seats on a real
...
What this settles for Plan:

- `assign --apply` seats the item and starts the tool's interactive session
  in a pseudo-terminal held by a Plan daemon. The daemon is the thing
...
- Trust is seeded the way Superset does it, into each CLI's own sanctioned
  store, and only for a folder Plan itself just created.
...
- **2026-09-08, first round.** `infra/lib/plan.mjs` owns the plan and
  ...
  `plan`; `skills/plan/` owns the pattern and the fields. `assign
  --apply` seats each chosen item as the Seat seat of the same id. Seats the
  ...
  and claim holds released by done reports: [evidence](evidence/2026-09-08-plan.md).
  `npm test` 243/243 (235 before), 11 guards reverted for red 11/11.
- **2026-09-08, second round.** `infra/lib/plan-serve.mjs` owns `serve`
  ...
  socket, both items committed and done in 74 s: [evidence](evidence/2026-09-08-plan.md).
  `npm test` 245/245, 9 more guards seen red. Session states and the local
  file's `tools` are owned by `skills/plan/references/plan.md`.
- **2026-09-08, third round.** `infra/lib/plan-hooks.mjs` owns `hooks`:
  one marked entry per event in the hook store of every tool found on the
  machine (Codex, Grok, Cursor, OpenCode), gated on `PLAN_EVENTS` so it
  ...
  [evidence](evidence/2026-09-08-plan.md). The tool-native paths
...
`plan plan` and know what is happening**, which is the thing this catalog has
