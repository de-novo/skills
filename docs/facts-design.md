# Facts — the facts under the forest

Written 2026-09-08. Status: landed the same day, first round. Once landed,
`infra/lib/facts.mjs` owns the behaviour and `skills/facts/references/log.md`
owns the fields. This note records why.

## The gap

Ground is the ground, Seat the seats, Plan the plan, Canopy the screen,
...
The other side of the gap is just as real. Plan must not become
...
proposed as a fact. Facts never writes the plan or a seat.
...
- **A file under the same state root as Seat**, keyed by the Seat slug. A
...
- **Point-in-time is `valid_from <= at < valid_to`** over committed facts,
  ...
  in the world.
- **No default writer.** A seat is `seat:<SEAT_ID>`; anyone else passes
...
- **Judges are declared in the values file, not authenticated.** `judges`
  ...
  trust Seat's registry already rests on. Authentication is a later
...
[facts-cases.md](facts-cases.md).

- Two writers appending in the same instant. Closed in the second round:
  every write re-reads the log under the file lock Seat's registry uses,
  and a race of four committers on a `one` predicate leaves one active
  (`infra/bin/facts.test.mjs`).
- A second machine. The log is machine-local like Seat's registry.
- A query language. Filters are exact matches, one moment (`--at`), one
  change horizon (`--since`), and one chain (`trace`).
- A hook that proposes on `seat report --status done` without a person
