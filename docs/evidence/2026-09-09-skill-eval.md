...
| Evidence | the session's own transcript under `~/.claude/projects/…/*.jsonl` (assistant text and tool calls), the Seat journal, `git show` in the seat |
| State | probe C ran with `GROUND_STATE_DIR` set to a scratch directory; the machine registries hold 0 mentions of the eval afterwards |
| Teardown | terminals closed, `seat finish eval-seat --apply`, `orca worktree rm --force`, seat branch deleted; the seat's commit was cherry-picked into this PR |

## Probe A: Plan grills before it plans

Sent: `/plan Goal: add a --since ISO flag to understory reading that
...
| --- | --- |
| Reads before asking | 14 tool calls first: the Understory reference and module, the Plan and Seat modules, the seat journal shape |
...
| Locked decisions to Facts | offered: "the nine answers are `decided` facts worth proposing", and held it because of the no-write instruction |
| No file written | `.agents/plan.yml` absent at the end; the session said so itself |

Verdict: the grilling, the vertical-slice rule, the quiz step, and the
Facts seam all fired from the skill text alone.
...
| Checked for | Observed |
| --- | --- |
| Reads the two screens first | one Bash call running `understory reading` and `facts query --brief` before any prose |
...
| Second run, dependencies present | `understory reading` → `plan not found`; `facts query --brief` → one active fact; the reply quoted both, said "What is done: nothing in the project", listed three open items, and ended with one next action |
...
Seat shape (outcome, boundary, verification, report, forbidden). A fresh
session was started in the seat's worktree with the `seat seat --shell`
environment and told only: "You woke up seated: SEAT_ID is set. Follow
the seat skill from the start."

| Checked for | Observed |
| --- | --- |
| Reads the skill and the task through the CLI | `cat`-equivalent Read of `$SEAT_SKILL`, then `seat seat "$SEAT_ID" --json` and `--task` |
...
| Commits on its branch, does not push | commit `bd4eb62` on `seat/eval-seat`; no push, no merge, no `finish` |
...
| Facts proposal | none; nothing in the task outlived the seat, and the log in the scratch state holds 0 rows |
...
the skill; Plan actually writing the plan and `assign --apply` seating
