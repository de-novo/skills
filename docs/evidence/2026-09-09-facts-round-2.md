# Facts round 2 in the playground sandbox — 2026-09-09

The second round carried the seven items named in
[`docs/facts-cases.md`](../facts-cases.md): declared predicates with
...
one execution of all of them inside a playground sandbox with a real Seat
...
| C2 | after `seat report --status blocked`, `propose --from-seat worker --report blocked` wrote a staging fact carrying the note; the same call without `--report` was refused for lacking a done report |
...
| F1 | `down`: 0 ports, 0 machine seat or overlay mentions, 0 directories remaining; machine Seat and Facts files hold 0 sandbox paths |
...
judges: [human:reader]
EOF
# a two-item plan (worker, reviewer depends_on worker) in $P/.agents/plan.yml
...
(cd $W && node $CLI facts propose --s worker --p caused_by --o x --s-type item --domain sample --source f)        # A13 refused
(cd $W && node $CLI facts propose --s reviewer --p depends-on --o worker --s-type item --o-type item --domain sample --source .agents/plan.yml)
(cd $W && node $CLI facts propose --s reviewer --p depends-on --o design-note --s-type item --o-type decision --domain sample --source docs/design.md)
node $CLI facts commit --project "$P" <e1> --by human:reader; node $CLI facts commit --project "$P" <e2> --by human:reader
(cd $W && node $CLI facts propose --s reviewer --p depends-on --o worker ...); node $CLI facts commit --project "$P" <dup> --by human:reader   # A5 refused
(cd $W && node $CLI facts propose --s sample-web --p uses --o 'plain html' ...); (cd $W && ... --o 'html plus script' ...)
node $CLI facts commit --project "$P" <u1> --by human:reader
node $CLI facts commit --project "$P" <u2> --by human:reader              # conflict, refused
node $CLI facts commit --project "$P" <u2> --by human:reader --supersede
node $CLI seat report worker --project "$P" --status blocked --note 'waits for the design note'
node $CLI facts propose --project "$P" --from-seat worker --report blocked --s-type seat --domain sample --by human:reader
node $CLI facts propose --project "$P" --from-seat worker --s-type seat --domain sample --by human:reader          # no done report, refused
node $CLI facts query --project "$P" --s sample-web --all
node $CLI facts trace --project "$P" <u2>
node $CLI facts query --project "$P" --since 2100-01-01T00:00:00Z
node $CLI facts query --project "$P" --all --since 2026-09-01T00:00:00Z
node $CLI facts query --project "$P" --brief --s reviewer
...
`node --test infra/bin/facts.test.mjs infra/bin/understory.test.mjs`:
...
flag passed to the wrong verb). It is now `infra/bin/facts-arc.test.mjs`
...
`seat report`. Unchanged from the design note.
