...
Plan grilling, a plan, two rounds of Seat seats (Claude Code and
...
| Vocabulary | `.agents/facts.yml`: domains `notes`, `sprint`; types `item decision module check seat`; predicates `decided caused-by depends-on passed failed tried-and-failed`; judge `human:jhpark` |
| Budget | `.agents/plan.local.yml`: `parallel: 2`, tools `claude` and `codex` |
| Skills in the project | copied into `.agents/skills` and `.claude/skills`, because the sandbox refuses symlinks (isolation rule 1) |
| Sessions | Claude Code 2.1.265 for the Plan and two seats; Codex 0.153.4 (`-a on-request -s workspace-write --add-dir <sandbox>`) for one seat; all in Orca terminals, driven with `terminal send` and `wait --for tui-idle`, approvals answered by the driver |
| Person | the driver, answering the grilling, committing facts as judge, merging, restarting the baseline |

## What happened, in order

1. **Grilling.** `/plan` with a three-sentence goal. The session read the
...
2. **Decisions into the log.** The session proposed twelve `decided` facts
   to Facts (source: the grilling round and the session id); the judge
...
3. **Plan.** The session wrote `.agents/plan.yml`, ran
   `plan plan` and `plan next`, and stopped. The driver committed
   the plan and ran `plan assign --apply`: `assigned 2/2 · slots 2/2`,
...
4. **Round one.** Each seat woke with the seat environment and one
   sentence ("You woke up seated: SEAT_ID is set…"). Both read
   `$SEAT_SKILL` and `seat seat --task` before anything else. Both
...
5. **Unblocking through the log.** The judge committed the seats' facts.
   The driver told the Codex seat one thing: read `facts query --brief
...
6. **Round two.** `plan assign --apply` seated `readme-notes-api`
   ...
   exercised (503).
7. **Merge and restart.** `plan plan`: `items 3 · done 3`. The driver
   ...
   DELETE answered 405.
8. **The record.** `/understory` in the Plan session wrote
...
9. **Finish and down.** `seat finish --apply` ×3: worktrees removed,
   branches kept, journals archived; overlays 0. `playground down`: ports
   0, machine mentions 0, directories remaining 0. Machine Seat and
   Facts files hold 0 mentions of the sandbox.
...
- **Product defect, fixed in this PR.** `playground/tools/lib/sandbox.mjs`
  ...
  `build` and `overlay` refused `GROUND_STATE_DIR` as outside the sandbox.
...
- **Sandbox rule met the planting rule.** Isolation rule 1 refuses
  ...
  skill body. Inside a throwaway sandbox the copy wins; the Plan
...
- **Driver friction.** Claude Code's `--allowedTools` patterns do not cover
  ...
  person answered roughly a hundred prompts. `plan serve` with its
  hooks is the designed path for this and was not used here so that every
  prompt could be read.
- **Prompt wording.** The Plan session first called the CLI as
  `de-novo skills plan`; the sandbox runs the catalog's `cli.mjs`
  directly, which takes the verb without `skills`. It corrected itself.

## Not measured

`plan serve` holding the sessions (Orca terminals were used instead);
