# Facts — the cases it must carry

Facts keeps a project's assertions in an append-only log. These cases
...
Start with the [Facts introduction](../skills/facts/README.md);
the [reference](../skills/facts/references/log.md) owns the CLI and log
contract, and the [design](facts-design.md) explains the choices.

A **seat** is a worker's Git worktree and task, managed by Seat. A
**judge** is a declared writer allowed to promote or invalidate assertions.
Plan holds the work plan; Understory renders its progress and links to
facts. A **slug** identifies a project's local state files. Every other
name a reader meets here (Ground, Seat, Herbarium, the catalog, the
...
- [First-round sandbox record](evidence/2026-09-08-facts-sandbox.md):
  initial lifecycle, writer identity, and isolation observations.
- [Second-round record](evidence/2026-09-09-facts-round-2.md):
...
- [Automated sandbox arc](../infra/bin/facts-arc.test.mjs) and
  [library and CLI tests](../infra/bin/facts.test.mjs): executable checks.
...
| # | Case | Mechanism today | Mark |
| --- | --- | --- | --- |
| B1 | A seat writes from its worktree with nothing set | writer from the Seat registry by worktree | measured |
| B2 | A seat writes with `SEAT_ID` | `seat:<id>` | tested |
...
| B9 | Inside a playground sandbox, where `SEAT_*` is stripped | worktree fallback | measured |
| B10 | A writer claims an id that is not theirs | not prevented; declared trust, same as Seat's registry | pattern |
...
| C6 | A new seat's brief should start from the facts | round 2: `query --brief` prints the block a Seat brief takes | measured |
| C7 | Understory should point at what was proven | round 2: `understory reading` lists the active fact ids per item, through Facts's query | measured |
...
| C9 | Plan's plan keeps work structure; Facts keeps world facts | separate files for the same project slug; report import reads the registry without changing it | tested |
...
| # | Case | Mechanism today | Mark |
| --- | --- | --- | --- |
| F1 | A sandbox run must not reach the machine log | state root from `GROUND_STATE_DIR`; dated records and the arc check for sandbox paths in machine registries | measured |
| F2 | The catalog's own facts | its own slug, `.agents/facts.yml` here | measured |
