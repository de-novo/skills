# Ground consumer evaluation protocol
...
[product direction](../ground-product-direction.md).
...
consumer commands or retaining raw evidence. Generic protocol and Ground-owned
...
- Existing procedure and Ground readiness, inspected without mutation first.
...
5. Apply the same independent completion check to both methods. A command's exit
   code or Ground receipt alone does not prove the comparison's runtime result.
