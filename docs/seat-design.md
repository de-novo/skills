...
owned by [`skills/seat/SKILL.md`](../skills/seat/SKILL.md), the schema,
registry, and CLI by [`skills/seat/references/seats.md`](../skills/seat/references/seats.md), and
the checks by `infra/lib/seat.mjs`. This document keeps only the reasons for
the decisions. Where the implementation differs from the design: under
`GROUND_STATE_DIR` the registry also goes into a `seats/` subdirectory, so it
does not overlap Ground's files. `report`'s `--session` overwrites the seat's
value. `status` reads Ground `overlay status --json` to count whether an env is
tracked and what work is in progress.

## In one sentence

Ground is the ground. Seat makes a **seat** on one tree (one worktree) of that
...
which seat, and with which tool, is decided by a person. Seat does not start a
worker.
...
| Question | Owner |
| --- | --- |
| Names, engines, overlay env, lease, the applied judgment | Ground |
| Creating, registering, and clearing a seat (worktree + env + task text), the seat count and the history | Seat |
| Which task in which seat, with which tool | A person |
| Running the agent process, continuing a conversation, detecting idleness and permissions | The launcher (a terminal, a worktree manager app, tmux, an ACP client, and so on). Outside Seat |
| Task DAGs, supervision loops, mailboxes, escalation | The orchestrator. Outside Seat |
| Branch merging, pushing, review | People and the project. Seat does not do them |
| Browser QA, e2e | The project. Outside both skills |

The dependency runs one way. Seat uses Ground. Ground does not know about Seat
and works unchanged without it. In the other direction, Seat has to work with
...
The contact with Ground is three public CLIs only.
...
Seat walks have to be the same, so that a result measured at one boundary is
valid for both. The cost of starting itself as a child process is accepted.

## Why Seat does not start an agent
...
- Both apps checked create the worktree themselves. If Seat monopolized
  worktree creation, it would fight these tools.

So Seat's output is a **seat**. A seat is a bundle of values that can be handed
to any launcher as it is, and the launcher is not a value and is outside Seat.

## Vocabulary

- **seat**: one worktree + one overlay env (when there is an overlay) + the task
  text + the `SEAT_*` environment variables. The id is a DNS label and is the
  same as the overlay env name. The Ground registry and the Seat registry meet
  on the same key.
- **seat**: the worker sitting in a seat. It may be a person, or an agent
  started with some tool. Seat only writes down the name tag (`--by`) of who
  sat there.
...
- **journal**: the lines that pile up in time order for each seat. What Seat
...
- **baseline checkout**: the original directory where the project was cloned. A
  seat is not created here. This is the same line as Ground's "do not start the
  baseline from an agent worktree".

## The values file

`.agents/seat-profile.yml`, committed to the project repository. The top-level
...
worktrees:
  root: ../<slug>-seats         # where Seat creates them. Relative to the baseline, and must be outside the repository
  branch: "seat/{id}"           # the only placeholder allowed is {id}. The result must be a valid git ref
```

The reason there are only these values is that the launcher is outside. Which
tool to start with which flags is the launcher's configuration, not Seat's
...
`~/.dev-infra/seats/<slug>.yml`. The same directory rules as the Ground overlay
registry (respect `GROUND_STATE_DIR`, rename a temporary file, private
...
seats:
  w1:
    worktree: /Users/me/acme-seats/w1
    ...
    journal:
      - { at: 2026-09-06T02:10:00Z, actor: seat, event: plan, detail: "worktree add seat/w1 at 0123456" }
      - { at: 2026-09-06T02:10:04Z, actor: seat, event: overlay.create, detail: "ok env w1" }
...
`journal` is append-only. `actor: seat` is what Seat did, `actor: seat` is
...
| What was changed in the code | The branch's commits. `git log <base>..seat/w1` |
| What was attached to and detached from the shared environment | The Ground overlay registry. Per env: the mutation, the image, the time, and the worktree that made it |
| When it went through which state | The journal |
| Exactly what happened inside the tool | The tool's native log that `session` points at. Seat does not open it |
...
a shared DB directly, and so on — the Ground rules forbid it, but it is not
...
```text
de-novo skills seat plan   <id> (--task <text> | --task-file <path>) [--worktree <existing-path>] [--by <label>] [--project <root>] [--apply]
de-novo skills seat seat   <id> [--json | --env | --shell] [--project <root>]
de-novo skills seat report <id> --status <working|blocked|done> [--note <text>] [--session <ref>] [--project <root>]
de-novo skills seat status [id] [--project <root>] [--json]
de-novo skills seat finish <id> [--project <root>] [--apply]
```

If `--project` is omitted, it walks up from cwd looking for
`.agents/seat-profile.yml`. When a worker calls it inside a worktree, the
worktree's `.agents/` is what gets found, so the baseline path is passed through
`SEAT_PROJECT`. The registry slug is read from that baseline's
`runtime-profile.yml` `project.slug`. The Seat profile has no slug. This is so
...
is the same shape as Ground's pending recovery.
...
- `--env`: four lines in the form `SEAT_ID=w1`
- `--shell`: one line, `cd '<worktree>' && export SEAT_ID='w1' SEAT_ENV='w1' SEAT_BRANCH='seat/w1' SEAT_PROJECT='<baseline>'`. Values are wrapped in single quotes and the quotes are escaped.

`SEAT_ENV` is an empty string when there is no env. The task text is not put
...
■ acme — seats 3
  ...
  w1  seat/w1  +12  env w1 ready    done      by claude
  w2  seat/w2  +3   env w2 ready    working   by codex
  w3  seat/w3  0    -               blocked   by human   "orders schema decision needed"
```
...
capture `status <id>` to a file before finish. Seat is not an archive.

## Examples of handing a seat to a launcher

Seat runs none of the following. A person or an orchestrator does.

```bash
# a person, directly in a terminal
eval "$(de-novo skills seat seat w1 --shell)" && claude

# a launcher that creates the worktree first: adopt that worktree
de-novo skills seat plan w2 --worktree <launcher-created-path> --task-file tasks/w2.md --by codex --apply

# a script: read the seat JSON and use whichever tool you want
de-novo skills seat seat w3 --json | my-launcher --stdin
```

## When a worker wakes up

Whatever the launcher, the worker starts with the worktree as cwd and the
`SEAT_*` environment variables set. The task text goes in as the first prompt,
so no file is written inside the worktree. Git status has to start clean so that
only the worker's changes remain.

An outline of `skills/seat/SKILL.md`:

```text
frontmatter  name: seat, description: triggers are the presence of SEAT_ID, /seat, "seat"
# Seat
One sentence. This file owns the pattern; values live in .agents/seat-profile.yml.
## You are a seat when
When SEAT_ID is set. Read your own seat with seat --json. Otherwise ignore this section.
## Rules
1. Do not change anything outside your worktree. Not the baseline, not other seats.
2. Verify with the /ground procedure. The env already exists. attach is yours, destroy is finish.
...
launcher's completion event is the project's or the launcher's choice, and Seat
does not require it.
...
- Seat does not start an agent process. The launcher is outside.
...
- Ground is called only through the public CLI.
- No merging, pushing, or reviewing.
- Success is counted. `seats n`, `worktrees n/n`, `envs n/n`, `reported …`.

## The state on Ground's side (checked 2026-09-06, a correction)
...
and `infra/bin/overlay-parallel.test.mjs` and went into `b794a82`. Ground now
records pending per env and takes a per-env lock, so different envs do not block
each other. Seat uses the seat id as the env name, so two seats never touch the
same env. **A concurrent attach from two seats can be measured with no
precondition on Ground's side.**

What is left is not blocking but reporting. Even on an applied mutation that is
proceeding normally, Ground writes pending before dispatch and removes it after
...
to an abort. Seat `status` counts this as an "overlay status returned non-zero"
problem. For a person counting other seats while one seat is attaching, that is
a false alarm. The seat that fixes it is Ground's house. If it marked
`pending-item … in-flight` when the owning pid of the env lock file is alive and
`stalled` otherwise, Seat could stop counting in-flight as a problem. The exit
code contract stays as it is.

## Measurement plan

`infra/bin/seat.test.mjs`. A temporary git repository as the baseline, a
temporary directory as `GROUND_STATE_DIR`, and Ground's process-workload fixture
...
real tool (claude, codex, and so on) is outside Seat, so it is out of scope
rather than `notMeasured`. Verifying two seats attaching at the same time sits
on top of the path Ground's parallel overlay already measured, so it can be
...
1. `infra/lib/seat.mjs`: the profile parser and the registry read/merge/write.
   Tests.
2. `plan` (both paths), `seat`, `report`, `status`. Wire up the CLI verbs. Tests.
3. `finish`. Tests.
4. `skills/seat/SKILL.md`, `README.md`, `examples/`, the `.agents/skills/seat`
...
5. A harness for two seats attaching at the same time. The in-flight/stalled
   marking in Ground's `overlay status` is a separate Ground PR.
...
[the evidence record](evidence/2026-09-06-seat.md).

- `seat` hands out `SEAT_SKILL` (the path to SKILL.md inside the catalog). In a
...
- Rule 1 was changed to "reading is allowed, writing is not", and rule 5 to
  "check before done".
- (2026-09-07) A project without Ground. The design said "it works with worktrees
  alone on a project without an overlay", but the implementation read the slug
  only from the runtime-profile and rejected the project when there was no Ground
  profile. It came out while trying to use Seat on the catalog itself. Now the
  seat-profile may carry `project.slug` only when there is no runtime-profile,
  and having both is rejected as a duplicate.
- (2026-09-07) The project index and seat hostnames. A tool had no way to know
  which project uses Seat, and the seat's env name alone did not give an
  ...
  gets `hostnames`, whose value is read from Ground's `urls --json`. Seat does
...
again by name alone, and Ground's create, attach, readiness observation, and
...
- **Waiting for completion.** An orchestrator polls `status --json` and waits for
  `done`. `seat wait <id> --for done|blocked --timeout` would be convenient, but
  it touches the "Seat does not supervise" boundary. Look at it when polling
...
- **A supervision mailbox.** Things like `worker_done`, `ask`, and `escalation`
  are an orchestrator's job. Seat `report` is not a smaller version of that; it
...
The npm `seat` is a package stopped at 0.0.0, and Microsoft Research Seat is
archived data-parallel research (checked 2026-09-06). This skill is not an npm
package and its CLI is `de-novo skills seat`, so there is no collision.
...
- Importing Ground's internal modules.
- Adding a key to `runtime-profile.yml`. Duplicating the slug in the Seat
  profile.
